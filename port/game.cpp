...
void TimerInit(void)
{
	UnsignedWide			ms;								// Store the result from Microseconds()

	Microseconds(&ms);										// Get the number of Microseconds since startup
	 
	memset(&timer, 0, sizeof(timer));						// Clear Our Timer Structure

	timer.mm_timer_start	= ms.lo;						// Use the lower part of the ms for our counter
...
float TimerGetTime()
{
	...
	return( (float) (ms.lo - timer.mm_timer_start) * 1000.0f);
}

short OpenNewFile( SFReply *psfReply, OSType  osTypeCreator, OSType  osTypeType );
short OpenNewFile( SFReply *psfReply,
...
void LoadGame(Str255 Name, int animnum)
{
  ...
  FSClose( sFile );
  if (localframenum==20){righthighleg[0][0][0]=100;}
}
...
void	LoadGLTexture( char *fileName, GLuint num )
{
	TGAImageRec	*tempTexture; // Change this to an AUX_RGBImageRec for Windows
	texture[num]=0;
	
	//glDeleteTextures( 1, &texture[num] );
	tempTexture = LoadTGA( fileName ); // Change this function to auxDIBImageLoad( filename ) for Windows
	if(tempTexture){
	glPixelStorei( GL_UNPACK_ALIGNMENT, 1 );
	
	if(texture[num]==0){glGenTextures( 1, &texture[num] );}
	...
	//glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB, tempTexture->sizeX, tempTexture->sizeY, 0, GL_RGB, GL_UNSIGNED_BYTE, tempTexture->data );
	...
	if(num==38||num==39){glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );}
	if(num==51||num==52){glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );}
	if(num==69||num==70||num==71){glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );}
	//if(num==53){glTexEnvi( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_BLEND );}
	/*if(num>=43&&num<=48){glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST );
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST );}*/
	gluBuild2DMipmaps( GL_TEXTURE_2D, GL_RGB, tempTexture->sizeX, tempTexture->sizeY, GL_RGB, GL_UNSIGNED_BYTE, tempTexture->data );
	free( tempTexture );}
}
/********************> ReSizeGLScene() <*****/
...
/********************> InitGL() <*****/
void setChannelSoundLevel(short leftVolume, short rightVolume,int which);
void setChannelSoundLevel(short leftVolume, short rightVolume,int which)
{
   SndCommand     theCmd;
   
   if (psndChannel[which])
   {
      theCmd.param1  = 0;
      theCmd.param2  = (long)((long) rightVolume << 16L |leftVolume);
      theCmd.cmd     = volumeCmd;
      SndDoImmediate(psndChannel[which], &theCmd);
   }
}

GLvoid playsound(int which, long volume);
GLvoid playsound(int which,long volume){
	if (volume<0){volume=0;}
	if (volume>200){volume=100;}
	if (which==13&&volume>100){volume=100;}
	unusedchannel++;
	if (unusedchannel==40){unusedchannel=0;}
	setChannelSoundLevel(volume,volume,unusedchannel);
	sndCommand.cmd = quietCmd;
	SndDoImmediate(psndChannel[unusedchannel], &sndCommand);
	SndPlay	(psndChannel[unusedchannel],(SndListHandle) hSound[which], (unsigned char) true);
}

GLvoid playsoundchannel(int which, long volume, int channel);
GLvoid playsoundchannel(int which,long volume, int channel){
	if (volume<0){volume=0;}
	setChannelSoundLevel(volume,volume,channel);
	sndCommand.cmd = quietCmd;
	SndDoImmediate(psndChannel[channel], &sndCommand);
	SndPlay	(psndChannel[channel],(SndListHandle) hSound[which], (unsigned char) true);
}
...
void MovePlayerData(int first, int second){
	...
	for(x=0;x<maxsprites;x++){
		if (sprite[x].owner==first){sprite[x].owner=second;}
	}
...
void RemovePlayer(int x){
	...
	if(jetglownum[x]){sprite[jetglownum[x]].type=0;}
	if(x<numplayers-1){
		for(a=x;a<numplayers-1;a++){
			if(jetglownum[a]){sprite[jetglownum[a]].type=0;}
			if(jetglownum[a+1]){sprite[jetglownum[a+1]].type=0;}
...
void MakeLineLightning(int howmany,float endoffset,float x1, float y1, float z1, float x2, float y2, float z2){
	...
	unusedlightning++;
	if(unusedlightning>=kMaxLightning){unusedlightning=0;}
	...
	if(Lightning[unusedlightning].tesselation<=5){Lightning[unusedlightning].tesselation=5;}
	if(Lightning[unusedlightning].tesselation>=95){Lightning[unusedlightning].tesselation=95;}
	if(Lightning[unusedlightning].tesselation>20/*&&(int)RangedRandom(0,3)==2*/){arc=RangedRandom(20,Lightning[unusedlightning].tesselation+20);}
	...
	for (counter=1;counter<Lightning[unusedlightning].tesselation;counter++){
		...
		if(absolute(counter-arc)<10&&arc!=0){Lightning[unusedlightning].LightningPoint[counter][1]+=((int)RangedRandom(0,1)*2-1)*absolute(counter-arc)/600;}
		...
		if(absolute(counter-arc)<10&&arc!=0){Lightning[unusedlightning].LightningPoint[counter][2]+=((int)RangedRandom(0,1)*2-1)*absolute(counter-arc)/600;}
	...
	if(z2!=z1){Lightning[unusedlightning].yrotation = atan((x2 - x1) / (z2 - z1))/6.28*360+180;}
    ...
    if(y2!=y1){  Lightning[unusedlightning].zrotation = atan((x2 - x1) / (y2 - y1))/6.28*360+180;}
...
GLvoid BuildFont(GLvoid)								// Build Our Font Display List
{
	...
	glBindTexture(GL_TEXTURE_2D, texture[71]);			// Select Our Font Texture
...
GLvoid glPrint(GLint x, GLint y, char *string, int set, float size)	// Where The Printing Happens
{
	if (set>1)
	{
		set=1;
...
int InitGL(GLvoid)										// All Setup For OpenGL Goes Here
{
	...
	if(fogtoggle){glEnable(GL_FOG);	}					// Enables GL_FOG
...
void DrawSabreGlow(int whichguy){
	...
	if(whichguy==0){glColor4f(1,.5,.5,brightness/255);}
	if(whichguy==1){glColor4f(.5,1,.5,brightness/255);}
	if(whichguy==2){glColor4f(.5,.5,1,brightness/255);}
	glDisable(GL_LIGHTING);
	glEnable(GL_BLEND);
	if(fogtoggle){glDisable(GL_FOG);}
	glDepthMask(0);
	...
	glBegin(GL_QUADS);						
		glTexCoord2f(0.0f, 0.0f); glVertex3f(-.3f*size,-.05f*size, 0.0f);
		glTexCoord2f(1.0f, 0.0f); glVertex3f( .3f*size,-.05f*size, 0.0f);
		glTexCoord2f(1.0f, 1.0f); glVertex3f( .3f*size, .05f*size, 0.0f);
		glTexCoord2f(0.0f, 1.0f); glVertex3f(-.3f*size, .05f*size, 0.0f);
	glEnd();
	glPushMatrix();
	...
	glPushMatrix();
		...
		glPushMatrix();
			...
			glBegin(GL_QUADS);						
				glTexCoord2f(0.0f, 0.0f); glVertex3f(-.020f*size,-.020f*size, 0.0f);
				glTexCoord2f(1.0f, 0.0f); glVertex3f( .020f*size,-.020f*size, 0.0f);
				glTexCoord2f(1.0f, 1.0f); glVertex3f( .020f*size, .020f*size, 0.0f);
				glTexCoord2f(0.0f, 1.0f); glVertex3f(-.020f*size, .020f*size, 0.0f);
			glEnd();
		glPopMatrix();
		...
		glPushMatrix();
            ...
            glTranslatef(M[12], M[13], M[14]);
			glBegin(GL_QUADS);						
				glTexCoord2f(0.0f, 0.0f); glVertex3f(-.013f*size,-.013f*size, 0.0f);
				glTexCoord2f(1.0f, 0.0f); glVertex3f( .013f*size,-.013f*size, 0.0f);
				glTexCoord2f(1.0f, 1.0f); glVertex3f( .013f*size, .013f*size, 0.0f);
				glTexCoord2f(0.0f, 1.0f); glVertex3f(-.013f*size, .013f*size, 0.0f);
			glEnd();
		glPopMatrix();
	glPopMatrix();
	if(invisible[whichguy]==0){glEnable(GL_LIGHTING);
	glDisable(GL_BLEND);
...
void DrawBody(int whichguy){
	...
	glPushMatrix();
	glRotatef(((hiprot[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(hiprot[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((hiprot[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(hiprot[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((hiprot[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(hiprot[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
	...
	glPushMatrix();
		...
		glRotatef(((torsorot[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(torsorot[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((torsorot[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(torsorot[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f);glRotatef(((torsorot[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(torsorot[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		glBindTexture(GL_TEXTURE_2D, texture[TorsoNum[whichguy]]);
		Torso();
		...
		glPushMatrix();
			...
			glRotatef(((head[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(head[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((head[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(head[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((head[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(head[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
			if(whichguy!=firstperson-1||firstperson==0){
			if(skin[whichguy]==4){glScalef(.8,.8,.8); Head2();}
			if(skin[whichguy]!=4){Head();}
			}
		glPopMatrix();
		glDisable(GL_CULL_FACE);
		glPushMatrix();
			...
			if(itemtype[whichguy]==1){ glScalef(100,100,100); LightSabre();}
			glBindTexture(GL_TEXTURE_2D, texture[41]);
			if(itemtype[whichguy]==0){Sword();}}
			glGetFloatv(GL_MODELVIEW_MATRIX,M);
		...
		glPushMatrix();
			...
			glTranslatef(-1.5f,3.0f,0.0f);
			glScalef(-.8,.8,.8);
			glRotatef(((righthigharm[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthigharm[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((righthigharm[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthigharm[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((righthigharm[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthigharm[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
			Upperarm();
			glTranslatef(0.0f,-2.5f,0.0f);
			glRotatef(((rightlowarm[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightlowarm[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((rightlowarm[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightlowarm[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((rightlowarm[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightlowarm[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
			glBindTexture(GL_TEXTURE_2D, texture[UpperArmNum[whichguy]]);
			Lowerarm();
			glTranslatef(0.0f,-1.8f,0.0f);
			glRotatef(((righthand[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthand[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((righthand[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthand[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((righthand[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthand[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
			glBindTexture(GL_TEXTURE_2D, texture[UpperArmNum[whichguy]]);
			Hand();
			glBindTexture(GL_TEXTURE_2D, texture[32]);
		...
		glPopMatrix();
		glRotatef(-torsorot[0][frame[whichguy]][anim[whichguy]],1.0f,0.0f,0.0f); glRotatef(-torsorot[1][frame[whichguy]][anim[whichguy]],0.0f,1.0f,0.0f); glRotatef(-torsorot[2][frame[whichguy]][anim[whichguy]],0.0f,0.0f,1.0f);
		glTranslatef(0.0f,-0.5f,0.0f);
	glPopMatrix();
	glPushMatrix();
		glTranslatef(0.8f,0.0f,0.0f);
		glRotatef(((lefthighleg[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(lefthighleg[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((lefthighleg[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(lefthighleg[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((lefthighleg[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(lefthighleg[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		glBindTexture(GL_TEXTURE_2D, texture[UpperArmNum[whichguy]]);
		Upperarm();
		glTranslatef(0.0f,-2.5f,0.0f);
		glRotatef(((leftlowleg[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftlowleg[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((leftlowleg[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftlowleg[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((leftlowleg[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftlowleg[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		glBindTexture(GL_TEXTURE_2D, texture[LowerLegNum[whichguy]]);
		Lowerleg();
		glTranslatef(0.0f,-2.5f,0.0f);
		glRotatef(((leftfoot[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftfoot[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((leftfoot[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftfoot[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f);glRotatef(((leftfoot[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftfoot[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
	...
	glPushMatrix();
		glTranslatef(-0.8f,0.0f,0.0f);
		glRotatef(((righthighleg[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthighleg[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((righthighleg[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthighleg[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((righthighleg[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthighleg[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		glBindTexture(GL_TEXTURE_2D, texture[UpperArmNum[whichguy]]);
		Upperarm();
		glTranslatef(0.0f,-2.5f,0.0f);
		glRotatef(((rightlowleg[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightlowleg[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((rightlowleg[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightlowleg[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((rightlowleg[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightlowleg[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		glBindTexture(GL_TEXTURE_2D, texture[LowerLegNum[whichguy]]);
		Lowerleg();
		glTranslatef(0.0f,-2.5f,0.0f);
		glRotatef(((rightfoot[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightfoot[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((rightfoot[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightfoot[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f);glRotatef(((rightfoot[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightfoot[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		glBindTexture(GL_TEXTURE_2D, texture[ShoesNum[whichguy]]);
		glScalef(.8,.8,.8);
		Foot();
	glPopMatrix();
...
void DrawGuy(int whichguy, int sabre){
	...
	if(invisible[whichguy]&cloaktime[whichguy]>=200)
	{
		...
		if(sabre==0){DrawBody(whichguy);}
		if(sabre==1){DrawSabreBody(whichguy);}
		glEnable(GL_LIGHTING);
		glPolygonMode( GL_FRONT_AND_BACK, GL_FILL);
		glDisable(GL_BLEND);
		if(fogtoggle){glEnable(GL_FOG);}
	...
	if(invisible[whichguy]==0||cloaktime[whichguy]<100)
	{
		...
		if(cloaktime[whichguy]<100){
			...
			if(fogtoggle){glDisable(GL_FOG);}
			glDisable(GL_LIGHTING);
			if(invisible[whichguy]==1){glColor4f( 1.0, 1.0, 1.0, cloaktime[whichguy]/100 );}
			if(invisible[whichguy]==0){glColor4f( 1.0, 1.0, 1.0, 1-cloaktime[whichguy]/100 );}
		}
		if(sabre==0){DrawBody(whichguy);}
		if(sabre==1){DrawSabreBody(whichguy);}
		glDisable(GL_BLEND);
...
void HandleSpriteTypes(int x){
	...
	switch( spritetype )
	{
		...
		case smokesprite:
			...
			//glRotatef(sprite[x].spin,0.0f,-1.0f,0.0f);
			glColor4f(255,255,255,sprite[x].brightness/255);
			if(sprite[x].type2==1){sprite[x].size+=multiplier*gamespeed/120*2;}
			break;
		case exploflame:
			glRotatef(sprite[x].spin,0.0f,0.0f,1.0f);
			//glRotatef(sprite[x].spin,0.0f,-1.0f,0.0f);
...
void glDrawBigCube(float xWidth,float yWidth,float zWidth,int tesselation,float movement){
	int normallength=.3;
	if(theme==matrixtheme){normallength=1.0;}
	glBegin(GL_QUADS);
		// Front Face
		glNormal3f( 0.0f, 0.0f, -normallength);
		glTexCoord2f(0.0f, movement); glVertex3f(-xWidth, -yWidth,  zWidth);
		glTexCoord2f(tesselation, movement); glVertex3f( xWidth, -yWidth,  zWidth);
		glTexCoord2f(tesselation, tesselation+movement); glVertex3f( xWidth,  yWidth,  zWidth);
		glTexCoord2f(0.0f, tesselation+movement); glVertex3f(-xWidth,  yWidth,  zWidth);
		// Back Face
		glNormal3f( 0.0f, 0.0f,normallength);
		glTexCoord2f(tesselation, movement); glVertex3f(-xWidth, -yWidth, -zWidth);
		glTexCoord2f(tesselation, tesselation+movement); glVertex3f(-xWidth,  yWidth, -zWidth);
...
void glEnvironmentCube(int front, int left, int back, int right, int up, int down, int size){
	...
	if((yrot>=90&&yrot<=270)||(firstperson==1)){glBindTexture(GL_TEXTURE_2D, front);}
	...
	glBegin(GL_QUADS);
		// Front Face
		glNormal3f( 0.0f, 0.0f, -normallength);
		glTexCoord2f(beginningmargin, movement); glVertex3f(-xWidth, -yWidth,  zWidth);
		glTexCoord2f(tesselation, movement); glVertex3f( xWidth, -yWidth,  zWidth);
		glTexCoord2f(tesselation, tesselation+movement); glVertex3f( xWidth,  yWidth,  zWidth);
		glTexCoord2f(beginningmargin, tesselation+movement); glVertex3f(-xWidth,  yWidth,  zWidth);
	glEnd();
	if(yrot>=270||yrot<=90||firstperson==3||firstperson==0){glBindTexture(GL_TEXTURE_2D, back);}
	glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP );
	glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP );
	glBegin(GL_QUADS);
		// Back Face
		...
		glTexCoord2f(beginningmargin, movement); glVertex3f( xWidth, -yWidth, -zWidth);
	glEnd();
	if((xrot>=180&&xrot<=360)||(firstperson==1||firstperson==2)){glBindTexture(GL_TEXTURE_2D, up);}
	glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP );
	glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP );
	glBegin(GL_QUADS);
		// Top Face
		...
		glTexCoord2f(tesselation, tesselation+movement); glVertex3f( xWidth,  yWidth, -zWidth);
	glEnd();
	if((xrot<=180&&xrot>=0)||(firstperson==1||firstperson==2)){glBindTexture(GL_TEXTURE_2D, down);}
	glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP );
	glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP );
	glBegin(GL_QUADS);
		// Bottom Face
		...
		glTexCoord2f(tesselation, movement); glVertex3f(-xWidth, -yWidth,  zWidth);
	glEnd();
	if((yrot>=0&&yrot<=180)||(firstperson==1||firstperson==2)){glBindTexture(GL_TEXTURE_2D, right);}
	glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP );
	glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP );
	glBegin(GL_QUADS);
		// Right face
		...
		// Left Face
	glEnd();
	if((yrot>=180&&yrot<=360)||(firstperson==1||firstperson==2)){glBindTexture(GL_TEXTURE_2D, left);}
	glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP );
	glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP );
	glBegin(GL_QUADS);
		glNormal3f(normallength, 0.0f, 0.0f);
...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	if(firstperson&&guytrack==0){guytrack=1;}
	if(guytrack&&numplayers>0){
		...
		if(numplayers>1){
			for(a=0;a<numplayers;a++){
				for(b=0;b<numplayers-1;b++){
					...
					if(dist>longestdist){longestdist=dist; longestydist=ydist;}
				}
			}
		}
		if(numplayers==1){
		...
		if(firstperson==0||firstperson==3){
			xrot=0;
			if(firstperson==3){yrot=0; camerax=xpos; cameray=ypos; if(targetz>-450){z=targetz/2;}}
			if(longestydist>=100){xrot=(longestydist-100)/5; if(targetxrot>65){targetxrot=65;} if(xrot>65){xrot=65;}}
			if(targetz<-400){targetz=-400;}
	...
	glPushMatrix();
		...
		if(theme==marbletheme){glDrawBigCube(470,465,465,3,10);}
		if(theme==trontheme){glDrawBigCube(467,464,464,30,10);}
		if(theme==woodtheme){glDrawBigCube(470,465,465,4,10);}
		if(theme==metaltheme){glDrawBigCube(470,465,465,1,10);}
	...
	glPushMatrix();
		...
		for (x=20;x<80;x++){
			...
			for (y=20;y<80;y++){
				...
				if(x!=64||Map[y][x]!=1){	
				if (Map[y][x]==1){
					if(theme==matrixtheme||theme==shinytheme){glDisable(GL_LIGHTING); glColor4f(255.0, 255.0, 255.0, 1.0); glDrawFloor(5,.9,5,slide); glEnable(GL_LIGHTING);}
					if(theme!=matrixtheme&&theme!=shinytheme){Floor(0);}
					}
				if (Map[y][x]==2){
					if(theme==matrixtheme||theme==shinytheme){ glDisable(GL_LIGHTING); glColor4f(300.0, 300.0, 300.0, 1.0); glDrawFloor(5,.9,5,slide); glEnable(GL_LIGHTING);}
					if(theme!=matrixtheme&&theme!=shinytheme){Floor(0);}
					glPushMatrix();
				...
				if (Walls[y][x]!=0){
					...
					glBindTexture(GL_TEXTURE_2D, themebasenum);
					if(theme==matrixtheme){glDisable(GL_LIGHTING); glColor4f(255.0, 255.0, 255.0, 1.0); glDrawFloor(1,10,4.5,slide); glEnable(GL_LIGHTING);}
					if(theme!=matrixtheme){Wall();}
					glBindTexture(GL_TEXTURE_2D, themebasenum);
					glPopMatrix();
				}
			}
	...
	for(x=0;x<numplayers;x++){
		if(invisible[x]==0){DrawGuys(x,0);}
	}
	for(x=0;x<numplayers;x++){
		if(invisible[x]==1){DrawGuys(x,0);}
	}
	...
	glPushMatrix();
		glBindTexture(GL_TEXTURE_2D, themebacknum);
		glTranslatef(0.0f,152.0f,0.0f);
		if(theme==marbletheme){glDrawBigCube(455,451.5,450,3,0);}
		if(theme==trontheme){glDrawBigCube(455,451.5,450,30,0);}
		if(theme==woodtheme){glDrawBigCube(455,451.5,450,4,0);}
		if(theme==metaltheme){glDrawBigCube(455,451.5,450,1,0);}
		if(theme==matrixtheme){
			//glDisable(GL_LIGHTING);
			glColor4f(200.0, 200.0, 200.0, 1.0);
		...
		for (x=0;x<100;x++){
			...
			for (y=0;y<100;y++){
				...
				if (selectedx==y&&selectedy==x&&mapeditor==1){
					...
					glDrawCube(6,11,6,1);
					if(wireframe==0){glPolygonMode( GL_FRONT_AND_BACK, GL_FILL);}
	...
	glPushMatrix();
			...
			LightDiffuse[3] = 0;
			//LightDiffuse[3] = 1;
			LightDiffuse[2] = 0; LightDiffuse[1]=lightbrightness[0]/255; LightDiffuse[0]=lightbrightness[0]/255;
			if (lightbrightness[0]<=0){lighton[0]=0;}
			if (lightbrightness[0]>0){lightbrightness[0]-=multiplier*3*gamespeed;}
			if(lighton[0]==1){glEnable(GL_LIGHT2);}
			if(lighton[0]==0){glDisable(GL_LIGHT2);}
	...
	glPushMatrix();
			...
			LightDiffuse[3] = 0;
			//LightDiffuse[3] = 1;
			LightDiffuse[2] = lightbrightness[1]/255; LightDiffuse[1]=0; LightDiffuse[0]=0;
			if (lightbrightness[1]<=0){lighton[1]=0;}
			if (lightbrightness[1]>0){lightbrightness[1]-=multiplier*gamespeed;}
			if(lighton[1]==1){glEnable(GL_LIGHT3);}
			if(lighton[1]==0){glDisable(GL_LIGHT3);}
	...
	for (y=0; y<numplayers; y++){
		...
		for(x=0;x<maxswordtrail-1;x++){
			a=x+1;
			if(a==maxswordtrail-1){a=0;}
			if(x!=unusedswordtrail[y]&&SwordTrail[y][0].brightness[a][3]!=0&&SwordTrail[y][0].brightness[x][3]!=0){
				glBegin(GL_TRIANGLE_STRIP);
					glTexCoord2f(0,0); glColor4f(SwordTrail[y][0].brightness[x][0],SwordTrail[y][0].brightness[x][1],SwordTrail[y][0].brightness[x][2],SwordTrail[y][0].brightness[x][3]);
					glVertex3f(SwordTrail[y][0].TrailPoint[x][0],SwordTrail[y][0].TrailPoint[x][1],SwordTrail[y][0].TrailPoint[x][2]);
					glTexCoord2f(1,0); glColor4f(SwordTrail[y][0].brightness[a][0],SwordTrail[y][0].brightness[a][1],SwordTrail[y][0].brightness[a][2],SwordTrail[y][0].brightness[a][3]);
					glVertex3f(SwordTrail[y][0].TrailPoint[a][0],SwordTrail[y][0].TrailPoint[a][1],SwordTrail[y][0].TrailPoint[a][2]);
					glTexCoord2f(0,1); glColor4f(SwordTrail[y][0].brightness[x][0],SwordTrail[y][0].brightness[x][1],SwordTrail[y][0].brightness[x][2],SwordTrail[y][0].brightness[x][3]);
					glVertex3f(SwordTrail[y][0].TrailPoint[x][3],SwordTrail[y][0].TrailPoint[x][4],SwordTrail[y][0].TrailPoint[x][5]);
					glTexCoord2f(1,1);glColor4f(SwordTrail[y][0].brightness[a][0],SwordTrail[y][0].brightness[a][1],SwordTrail[y][0].brightness[a][2],SwordTrail[y][0].brightness[a][3]);
		...
		for(x=0;x<maxswordtrail-1;x++){
			...
			if(x!=unusedswordtrail[y]&&SwordTrail[y][1].brightness[a][3]!=0&&SwordTrail[y][1].brightness[x][3]!=0){
				glBegin(GL_TRIANGLE_STRIP);
					glTexCoord2f(0,0); glColor4f(SwordTrail[y][1].brightness[x][0],SwordTrail[y][1].brightness[x][1],SwordTrail[y][1].brightness[x][2],SwordTrail[y][1].brightness[x][3]);
					glVertex3f(SwordTrail[y][1].TrailPoint[x][0],SwordTrail[y][1].TrailPoint[x][1],SwordTrail[y][1].TrailPoint[x][2]);
					glTexCoord2f(1,0); glColor4f(SwordTrail[y][1].brightness[a][0],SwordTrail[y][1].brightness[a][1],SwordTrail[y][1].brightness[a][2],SwordTrail[y][1].brightness[a][3]);
					glVertex3f(SwordTrail[y][1].TrailPoint[a][0],SwordTrail[y][1].TrailPoint[a][1],SwordTrail[y][1].TrailPoint[a][2]);
					glTexCoord2f(0,1); glColor4f(SwordTrail[y][1].brightness[x][0],SwordTrail[y][1].brightness[x][1],SwordTrail[y][1].brightness[x][2],SwordTrail[y][1].brightness[x][3]);
					glVertex3f(SwordTrail[y][1].TrailPoint[x][3],SwordTrail[y][1].TrailPoint[x][4],SwordTrail[y][1].TrailPoint[x][5]);
					glTexCoord2f(1,1);glColor4f(SwordTrail[y][1].brightness[a][0],SwordTrail[y][1].brightness[a][1],SwordTrail[y][1].brightness[a][2],SwordTrail[y][1].brightness[a][3]);
	...
	{	
		if(sprite[x].type!=nothing){
			...
			glPushMatrix();
				...
				if(sprite[x].type!=bouncesprite&&sprite[x].type!=water&&sprite[x].type!=grenade&&sprite[x].type!=chunk&&sprite[x].type!=exploflame&&sprite[x].type!=muzzleflashstars&&sprite[x].type!=lightningflashstars&&sprite[x].type!=smokesprite&&sprite[x].type!=invisiblestars2){HandleSpriteTypes(x);}
				...
				if(sprite[x].type!=lasersprite&&sprite[x].type!=tracersprite&&sprite[x].type!=grenade&&sprite[x].type!=chunk){
					...
					glBegin(GL_QUADS);						
						glTexCoord2f(0.0f, 0.0f); glVertex3f(-.3f*sprite[x].size,-.3f*sprite[x].size, 0.0f);
						glTexCoord2f(1.0f, 0.0f); glVertex3f( .3f*sprite[x].size,-.3f*sprite[x].size, 0.0f);
						glTexCoord2f(1.0f, 1.0f); glVertex3f( .3f*sprite[x].size, .3f*sprite[x].size, 0.0f);
						glTexCoord2f(0.0f, 1.0f); glVertex3f(-.3f*sprite[x].size, .3f*sprite[x].size, 0.0f);
					glEnd();
				...
				if(sprite[x].type==lasersprite){
					glPushMatrix();
					glRotatef(sprite[x].spin,0.0f,0.0f,1.0f);
					if(sprite[x].velx>0){glRotatef(180,0.0,1.0,0.0);}
					glBindTexture(GL_TEXTURE_2D, texture[38]);
					glBegin(GL_QUADS);						
						glTexCoord2f(0.0f, 0.0f); glVertex3f(-.6f*sprite[x].size,-.2f*sprite[x].size, 0.0f);
						glTexCoord2f(1.0f, 0.0f); glVertex3f( .6f*sprite[x].size,-.2f*sprite[x].size, 0.0f);
						glTexCoord2f(1.0f, 1.0f); glVertex3f( .6f*sprite[x].size, .2f*sprite[x].size, 0.0f);
						glTexCoord2f(0.0f, 1.0f); glVertex3f(-.6f*sprite[x].size, .2f*sprite[x].size, 0.0f);
					glEnd();
					glPushMatrix();
						glRotatef(90,1.0,0.0,0.0);
						glBindTexture(GL_TEXTURE_2D, texture[38]);
					...
					glPushMatrix();
						...
						glRotatef(-xrot,1.0f,0.0f,0.0f);
						glBegin(GL_QUADS);						
							glTexCoord2f(0.0f, 0.0f); glVertex3f(-.1f*sprite[x].size,-.1f*sprite[x].size, 0.0f);
							glTexCoord2f(1.0f, 0.0f); glVertex3f( .1f*sprite[x].size,-.1f*sprite[x].size, 0.0f);
							glTexCoord2f(1.0f, 1.0f); glVertex3f( .1f*sprite[x].size, .1f*sprite[x].size, 0.0f);
							glTexCoord2f(0.0f, 1.0f); glVertex3f(-.1f*sprite[x].size, .1f*sprite[x].size, 0.0f);
						glEnd();
						glPopMatrix();
				...
				if(sprite[x].type==tracersprite){
					...
					glBindTexture(GL_TEXTURE_2D, texture[38]);
					glBegin(GL_QUADS);						
						glTexCoord2f(0.0f, 0.0f); glVertex3f(-.6f*sprite[x].size,-.05f*sprite[x].size, 0.0f);
						glTexCoord2f(1.0f, 0.0f); glVertex3f( .6f*sprite[x].size,-.05f*sprite[x].size, 0.0f);
						glTexCoord2f(1.0f, 1.0f); glVertex3f( .6f*sprite[x].size, .05f*sprite[x].size, 0.0f);
						glTexCoord2f(0.0f, 1.0f); glVertex3f(-.6f*sprite[x].size, .05f*sprite[x].size, 0.0f);
					glEnd();
					glPushMatrix();
						glRotatef(90,1.0,0.0,0.0);
						glBindTexture(GL_TEXTURE_2D, texture[38]);
					...
					glPushMatrix();
						...
						glRotatef(-xrot,1.0f,0.0f,0.0f);
						glBegin(GL_QUADS);						
							glTexCoord2f(0.0f, 0.0f); glVertex3f(-.025f*sprite[x].size,-.025f*sprite[x].size, 0.0f);
							glTexCoord2f(1.0f, 0.0f); glVertex3f( .025f*sprite[x].size,-.025f*sprite[x].size, 0.0f);
							glTexCoord2f(1.0f, 1.0f); glVertex3f( .025f*sprite[x].size, .025f*sprite[x].size, 0.0f);
							glTexCoord2f(0.0f, 1.0f); glVertex3f(-.025f*sprite[x].size, .025f*sprite[x].size, 0.0f);
						glEnd();
						glPopMatrix();
						glTranslatef(-4.0f,0.0f,0.0f);
						glPushMatrix();
	...
	glRasterPos3d (.1, 0, 0); 
	if(showinfo==1)DrawCSringGL (aChar, gFontList);
	...
	glColor3d(1.0, 1.0, 1.0);
	glRasterPos3d (.1, 0, 0); 
	if(showinfo==2)DrawCSringGL (aChar, gFontList);
...
void 	restartRound(){
	...
	if(exists[1]==1){RemovePlayer(1);}
	if(exists[0]==1){RemovePlayer(0);}
	if(exists[0]==0){MovePlayerData(0,oldguy[0]);}
	if(exists[1]==0){MovePlayerData(1,oldguy[1]);}
	goneplayers=0;
	holding[0]=0;
	holding[1]=0;
	exists[0]=1;
	...
	if (randomint2>=randomint){randomint2++;}
...
void	HandleKeyDown( char theChar )
{
	...
	if(mapeditor){
		...
		case '8':
			...
			break;
		case '5':
			if(selectedy<99){selectedy++;}
			break;
		case '4':
			if(selectedx>0){selectedx--;}
			break;
		case '6':
			if(selectedx<99){selectedx++;}
			break;
		case '1':
		...
		case '7':
			playerstartnum++;
			if(playerstartnum>=16){playerstartnum=0;}
			break;
		case '9':
	...
	switch( theChar )
	{
		...
		case 'A':
			...
			break;
		case 9:
			if(firstperson==1){firstperson=2;}
			if(firstperson==0){firstperson=1; ReSizeGLScene(screenwidth,screenheight,90);}
			if(firstperson==2){firstperson=0; ReSizeGLScene(screenwidth,screenheight,45); yrot=0;}
			break;
		case '-':
			if(changablejump==1){jumpstrength++;}
			break;
		case '*':
			if(changablejump==1){jumpstrength--;}
			break;
		case '=':
			if(changablejump==1){gravity-=.005;}
			break;
		case '/':
			if(changablejump==1){gravity+=.005;}
			break;
		case 'L':
			...
			if(lighting==1){glEnable(GL_LIGHT1);}
			if(lighting==0){glDisable(GL_LIGHT1);}
			break;
		case 'c':
			if(changablejump==1){changablejump=2;}
			if(changablejump==0){changablejump=1;}
			if(changablejump==2){changablejump=0;}
			break;
		case 'C':
		...
		case 'Z':
			//for(x=0;x<numplayers;x++){	
				itemnum[0]++;
				if(itemnum[0]==4&&itemtype[0]==0){itemnum[0]=2;itemtype[0]=1;}
				if(itemnum[0]==4&&itemtype[0]==1){itemtype[0]=0;}
				if(itemnum[0]==7&&itemtype[0]==0){itemtype[0]=1; itemnum[0]=6;}
				if(itemnum[0]==7&&itemtype[0]==1){itemtype[0]=0;}
				if(itemnum[0]>7){itemnum[0]=0;}
			//}
			break;
		case 'V':
			//for(x=0;x<numplayers;x++){
				itemnum[1]++;
				if(itemnum[1]==4&&itemtype[1]==0){itemnum[1]=2;itemtype[1]=1;}
				if(itemnum[1]==4&&itemtype[1]==1){itemtype[1]=0;}
				if(itemnum[1]==7&&itemtype[1]==0){itemtype[1]=1; itemnum[1]=6;}
				if(itemnum[1]==7&&itemtype[1]==1){itemtype[1]=0;}
				if(itemnum[1]>7){itemnum[1]=0;}
			//}
			break;
		...
		/*case 'c':
			...
			break;*/
		case 'F':
			if (fogtoggle==0){fogtoggle=2;}
			if (fogtoggle==1){fogtoggle=0; glDisable(GL_FOG);}
			if (fogtoggle==2){fogtoggle=1; glEnable(GL_FOG);}
			break;
		case 'x':
			/*if (invisible==0){invisible=2;}
			if (invisible==1){invisible=0;}
			...
			break;
		case 'u':
			if (jetpack[0]==0){jetpack[0]=2;}
			if (jetpack[0]==1){jetpack[0]=0; usingjetpack[0]=0; if(activity[0]==jetidle||activity[0]==jetleft||activity[0]==jetup||activity[0]==jetright){activity[0]=falling;}}
			if (jetpack[0]==2){jetpack[0]=1;}
			if (jetpack[1]==0){jetpack[1]=2;}
			if (jetpack[1]==1){jetpack[1]=0; usingjetpack[1]=0; if(activity[1]==jetidle||activity[1]==jetleft||activity[1]==jetup||activity[1]==jetright){activity[1]=falling;}}
			if (jetpack[1]==2){jetpack[1]=1;}
			break;
		case 'U':
			unlimitedammo=1-unlimitedammo;
			break;
		case 'y':
			usingjetpack[0]=0; if(activity[0]==jetidle||activity[0]==jetleft||activity[0]==jetup||activity[0]==jetright){activity[0]=falling;}
			break;
		case '3':
			usingjetpack[1]=0; if(activity[1]==jetidle||activity[1]==jetleft||activity[1]==jetup||activity[1]==jetright){activity[1]=falling;}
			break;
		case 'X':
			if (trail==0){trail=2;}
			if (trail==1){trail=0;}
			...
			break;
		case 'v':
			if (guytrack==0){guytrack=2;}
			if (guytrack==1){guytrack=0;}
			if (guytrack==2){guytrack=1;}
			break;
		case 'b':
			if (slowdown==0){slowdown=2;}
			if (slowdown==1){slowdown=0;}
			if (slowdown==2){slowdown=1;}
			break;
		case 'B':
			if (freezetime==0){freezetime=2;}
			if (freezetime==1){freezetime=0;}
			if (freezetime==2){freezetime=1;}
			break;
		case 'm':
			if (wireframe==0){wireframe=2;}
			if (wireframe==1){wireframe=0;}
			if (wireframe==2){wireframe=1;}
			if (wireframe==1){glPolygonMode( GL_FRONT_AND_BACK, GL_LINE );}
			if (wireframe==0){glPolygonMode( GL_FRONT_AND_BACK, GL_FILL );}
			break;
		case 'M':
			if (mapeditor==0){mapeditor=2;}
			if (mapeditor==1){mapeditor=0;}
			if (mapeditor==2){mapeditor=1;}
			break;
		case 'n':
			theme++;
			if(theme>7){theme=0;}
...
void 	MakeSprite(float x, float y, float z, float brightness, float type, float spin, float size, float spin2, float velx, float vely, float velz){
	...
	unusedsprite=a;
	if (unusedsprite>=maxsprites){unusedsprite=0;}
	...
	for(a=0;a<16;a++){
		if(jetglownum[a]==unusedsprite){jetglownum[a]=0;}
	}
}
...
void 	MakeNewSwordTrail(int whichguy){
	if(swordtrailtoggle[whichguy][0]==1||swordtrailtoggle[whichguy][1]==1){
		...
		if(unusedswordtrail[whichguy]>=maxswordtrail-1){UpdateSwordTrail(whichguy,unusedswordtrail[whichguy]);unusedswordtrail[whichguy]=0;}
...
void 	SwitchFrame(int whichguy)
{
		...
		time[whichguy] -= 100;
		frame[whichguy]=targetframe[whichguy]; rotation[whichguy]=targetrotation[whichguy]; anim[whichguy]=targetanim[whichguy];
		framenum[whichguy]=1;
 			 for (x=0;x<3;x++){
				for (y=0;y<20;y++){
			if(leftlowarm[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(lefthigharm[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(rightlowarm[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(righthigharm[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(torsorotation[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(hiprot[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(torsorot[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(leftlowleg[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(lefthighleg[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(rightlowleg[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(righthighleg[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(hippos[2][y][targetanim[whichguy]]!=60){framenum[whichguy]=y+1;}
			if(lefthand[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(leftfoot[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(righthand[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(rightfoot[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(head[x][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(hippos[1][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
			if(hippos[0][y][targetanim[whichguy]]!=0){framenum[whichguy]=y+1;}
				}
			}
			
		if(invisible[whichguy]==0){
			if(frame[whichguy]==2&&activity[whichguy]==leftroll){ playsound(0,100);}
			if(frame[whichguy]==2&&activity[whichguy]==rightroll){ playsound(0,100);}
			if(frame[whichguy]==2&&activity[whichguy]==runleft){ playsound(1,100);}
			if(frame[whichguy]==2&&activity[whichguy]==runright){ playsound(1,100);}
			if(frame[whichguy]==0&&activity[whichguy]==runleft){ playsound(1,100);}
			if(frame[whichguy]==0&&activity[whichguy]==runright){ playsound(1,100);}
		}
		...
		if (anim[whichguy]==40&&frame[whichguy]==5){frame[whichguy]++; targetframe[whichguy]++;}
		if (activity[whichguy]==idle1||activity[whichguy]==idle2){targetframe[whichguy]+=RangedRandom(0,1)*2;}
		if (targetframe[whichguy]>=framenum[whichguy]){targetframe[whichguy]=0; if(activity[whichguy]==idle1||activity[whichguy]==idle2){activity[whichguy]=RangedRandom(1,2);}}
		if (activity[whichguy]==leftroll&&frame[whichguy]==5&&anim[whichguy]==8){frame[whichguy]=6; time[whichguy]=100;}
		if (activity[whichguy]==leftroll&&targetframe[whichguy]==0&&anim[whichguy]==8&&IsKeyDown( theKeyMap, kLeftKey[whichguy] )==0&&guyvely[whichguy]==0&&guyvelx[whichguy]==0){activity[whichguy]=ducking; targetframe[whichguy]=2;time[whichguy]=0;}
		if (activity[whichguy]==leftroll&&targetframe[whichguy]==0&&anim[whichguy]==8&&IsKeyDown( theKeyMap, kLeftKey[whichguy] )==1&&IsKeyDown( theKeyMap, kDownKey[whichguy] )==0&&guyvely[whichguy]==0&&guyvelx[whichguy]==0){activity[whichguy]=runleft; targetframe[whichguy]=1;time[whichguy]=0;}
		if (activity[whichguy]==leftroll&&targetframe[whichguy]==0&&anim[whichguy]==8&&guyvely[whichguy]!=0){activity[whichguy]=falling; targetframe[whichguy]=1;time[whichguy]=0; guyvelx[whichguy]=-.1;}
		if (activity[whichguy]==rightroll&&frame[whichguy]==5&&anim[whichguy]==8){frame[whichguy]=6; time[whichguy]=100;}
		if (activity[whichguy]==rightroll&&targetframe[whichguy]==0&&anim[whichguy]==8&&IsKeyDown( theKeyMap, kRightKey[whichguy] )==0&&guyvely[whichguy]==0&&guyvelx[whichguy]==0){activity[whichguy]=ducking; targetframe[whichguy]=2;time[whichguy]=0;}
		if (activity[whichguy]==rightroll&&targetframe[whichguy]==0&&anim[whichguy]==8&&IsKeyDown( theKeyMap, kRightKey[whichguy] )==1&&IsKeyDown( theKeyMap, kDownKey[whichguy] )==0&&guyvely[whichguy]==0&&guyvelx[whichguy]==0){activity[whichguy]=runright; targetframe[whichguy]=1;time[whichguy]=0;}
		if (activity[whichguy]==rightroll&&targetframe[whichguy]==0&&anim[whichguy]==8&&guyvely[whichguy]!=0){activity[whichguy]=falling; targetframe[whichguy]=1;time[whichguy]=0;guyvelx[whichguy]=.1;}
		if (activity[whichguy]==falling&&frame[whichguy]==4){targetframe[whichguy]=2;}
		if (activity[whichguy]==splat&&targetframe[whichguy]==0){time[whichguy]=0; activity[whichguy]=deadfacedown;}
		if (activity[whichguy]==impaledright&&anim[whichguy]==19&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=deadfacedown; time[whichguy]=0; health[whichguy]-=120;}
		if (activity[whichguy]==impaledleft&&anim[whichguy]==19&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=deadfacedown; time[whichguy]=0; health[whichguy]-=120;}
		if (activity[whichguy]==duckimpaledright&&anim[whichguy]==39&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=deadfacedown; time[whichguy]=0; health[whichguy]-=120;}
		if (activity[whichguy]==duckimpaledleft&&anim[whichguy]==39&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=deadfacedown; time[whichguy]=0; health[whichguy]-=120;}
		if (activity[whichguy]==jetimpaledright&&anim[whichguy]==37&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=deadfaceup; time[whichguy]=0; health[whichguy]-=120;}
		if (activity[whichguy]==jetimpaledleft&&anim[whichguy]==37&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=deadfaceup; time[whichguy]=0; health[whichguy]-=120;}
		if (activity[whichguy]==brokeneckright&&anim[whichguy]==17&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=deadfacedown; time[whichguy]=0; health[whichguy]-=120;}
		if (activity[whichguy]==brokeneckleft&&anim[whichguy]==17&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=deadfacedown; time[whichguy]=0; health[whichguy]-=120;}
		if (activity[whichguy]==swordthrownleft&&anim[whichguy]==21&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=deadfaceup; time[whichguy]=0; health[whichguy]-=120;}
		if (activity[whichguy]==swordthrownright&&anim[whichguy]==21&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=deadfaceup; time[whichguy]=0; health[whichguy]-=120;}
		if (activity[whichguy]==landonback&&targetframe[whichguy]==0&&anim[whichguy]==14){ time[whichguy]=0; activity[whichguy]=deadfaceup;}
		
		if (activity[whichguy]==getupfromback&&targetframe[whichguy]==0&&anim[whichguy]==24&&frame[whichguy]>0){ time[whichguy]=0; activity[whichguy]=idle1;}
		if (activity[whichguy]==getupfromfront&&targetframe[whichguy]==0&&anim[whichguy]==25&&frame[whichguy]>0){ time[whichguy]=0; activity[whichguy]=idle1;}
		
		if (activity[whichguy]==painoneright&&anim[whichguy]==22&&frame[whichguy]==5){guyx[whichguy]+=3;}
		if (activity[whichguy]==paintworight&&anim[whichguy]==23&&frame[whichguy]==5){guyx[whichguy]+=2;}
		if (activity[whichguy]==painoneright&&anim[whichguy]==22&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1; targetanim[whichguy]=1; targetframe[whichguy]=0; time[whichguy]=0;}
		if (activity[whichguy]==paintworight&&anim[whichguy]==23&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1; targetanim[whichguy]=1; targetframe[whichguy]=0; time[whichguy]=0;}
		if (activity[whichguy]==painthreeright&&anim[whichguy]==42&&frame[whichguy]==5){guyx[whichguy]+=4;}
		if (activity[whichguy]==painthreeright&&anim[whichguy]==42&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1; targetanim[whichguy]=1; targetframe[whichguy]=0; time[whichguy]=0;}
		
		if (activity[whichguy]==painoneleft&&anim[whichguy]==22&&frame[whichguy]==5){guyx[whichguy]-=3;}
		if (activity[whichguy]==paintwoleft&&anim[whichguy]==23&&frame[whichguy]==5){guyx[whichguy]-=2;}
		if (activity[whichguy]==painoneleft&&anim[whichguy]==22&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1; targetanim[whichguy]=1; targetframe[whichguy]=0; time[whichguy]=0;}
		if (activity[whichguy]==paintwoleft&&anim[whichguy]==23&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1; targetanim[whichguy]=1; targetframe[whichguy]=0; time[whichguy]=0;}
		if (activity[whichguy]==painthreeleft&&anim[whichguy]==42&&frame[whichguy]==5){guyx[whichguy]-=3;}
		if (activity[whichguy]==painthreeleft&&anim[whichguy]==42&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1; targetanim[whichguy]=1; targetframe[whichguy]=0; time[whichguy]=0;}
		
		if (activity[whichguy]==breakneckleft&&anim[whichguy]==16&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1;}
		if (activity[whichguy]==breakneckright&&anim[whichguy]==16&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1;}
		if (activity[whichguy]==swordrightthrust&&anim[whichguy]==27&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=swordidleright;}
		if (activity[whichguy]==swordleftthrust&&anim[whichguy]==27&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=swordidleleft;}
		if (activity[whichguy]==swordrightspin&&anim[whichguy]==40&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=swordidleright;}
		if (activity[whichguy]==swordleftspin&&anim[whichguy]==40&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=swordidleleft;}
		if (activity[whichguy]==swordrightthrust&&anim[whichguy]==28&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=swordidleright;}
		if (activity[whichguy]==swordleftthrust&&anim[whichguy]==28&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=swordidleleft;}
		if (activity[whichguy]==swordrightparry&&anim[whichguy]==29&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=swordidleright;}
		if (activity[whichguy]==swordleftparry&&anim[whichguy]==29&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=swordidleleft;}
		if (activity[whichguy]==impaleleft&&anim[whichguy]==18&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1;}
		if (activity[whichguy]==impaleright&&anim[whichguy]==18&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1;}
		if (activity[whichguy]==duckimpaleleft&&anim[whichguy]==38&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1;}
		if (activity[whichguy]==duckimpaleright&&anim[whichguy]==38&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1;}
		if (activity[whichguy]==jetimpaleleft&&anim[whichguy]==36&&targetframe[whichguy]==0&&frame[whichguy]>2){activity[whichguy]=jetidle; targetanim[whichguy]=35;}
		if (activity[whichguy]==jetimpaleright&&anim[whichguy]==36&&targetframe[whichguy]==0&&frame[whichguy]>2){activity[whichguy]=jetidle; targetanim[whichguy]=35;}
		
		if (activity[whichguy]==fallbackwardsright&&anim[whichguy]==13&&targetframe[whichguy]==0){activity[whichguy]=landonback; if(Walls[guymapx[whichguy]][guymapy[whichguy]]==0/*||Map[guymapx[whichguy]][guymapy[whichguy]]==1||Map[guymapx[whichguy]][guymapy[whichguy]]==2*/){guyx[whichguy]+=7*size[whichguy];} if(Walls[guymapx[whichguy]][guymapy[whichguy]]==1/*&&Map[guymapx[whichguy]][guymapy[whichguy]]!=1&&Map[guymapx[whichguy]][guymapy[whichguy]]!=2*/){guyvelx[whichguy]=-.1;} targetanim[whichguy]=14; anim[whichguy]=14; targetframe[whichguy]=0; frame[whichguy]=0; time[whichguy]=100; if(Map[guymapx[whichguy]][guymapy[whichguy]]!=0&&Map[guymapx[whichguy]][guymapy[whichguy]]!=3&&activity[whichguy]!=climbrope&&activity[whichguy]!=onrope&&activity[whichguy]!=downrope){playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);}}
		if (activity[whichguy]==fallbackwardsleft&&anim[whichguy]==13&&targetframe[whichguy]==0){activity[whichguy]=landonback; if(Walls[guymapx[whichguy]-1][guymapy[whichguy]]==0/*||Map[guymapx[whichguy]][guymapy[whichguy]]==1||Map[guymapx[whichguy]][guymapy[whichguy]]==2*/){guyx[whichguy]-=7*size[whichguy];} if(Walls[guymapx[whichguy]-1][guymapy[whichguy]]==1/*&&Map[guymapx[whichguy]][guymapy[whichguy]]!=1&&Map[guymapx[whichguy]][guymapy[whichguy]]!=2*/){guyvelx[whichguy]=.1;} targetanim[whichguy]=14; anim[whichguy]=14; targetframe[whichguy]=0; frame[whichguy]=0; time[whichguy]=100; if(Map[guymapx[whichguy]][guymapy[whichguy]]!=0&&Map[guymapx[whichguy]][guymapy[whichguy]]!=3&&activity[whichguy]!=climbrope&&activity[whichguy]!=onrope&&activity[whichguy]!=downrope){playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);}}
		if (activity[whichguy]==fallforwardsright&&anim[whichguy]==41&&targetframe[whichguy]==4){if(Map[guymapx[whichguy]][guymapy[whichguy]]!=0&&Map[guymapx[whichguy]][guymapy[whichguy]]!=3&&activity[whichguy]!=climbrope&&activity[whichguy]!=onrope&&activity[whichguy]!=downrope){playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);}}
		if (activity[whichguy]==fallforwardsleft&&anim[whichguy]==41&&targetframe[whichguy]==4){ if(Map[guymapx[whichguy]][guymapy[whichguy]]!=0&&Map[guymapx[whichguy]][guymapy[whichguy]]!=3&&activity[whichguy]!=climbrope&&activity[whichguy]!=onrope&&activity[whichguy]!=downrope){playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);}}
		
		if (activity[whichguy]==fallforwardsright&&anim[whichguy]==41&&targetframe[whichguy]==0){activity[whichguy]=deadfacedown; if(Walls[guymapx[whichguy]][guymapy[whichguy]]==1&&Map[guymapx[whichguy]][guymapy[whichguy]]!=1&&Map[guymapx[whichguy]][guymapy[whichguy]]!=2){guyvelx[whichguy]=-.1;}}
		if (activity[whichguy]==fallforwardsleft&&anim[whichguy]==41&&targetframe[whichguy]==0){activity[whichguy]=deadfacedown; if(Walls[guymapx[whichguy]-1][guymapy[whichguy]]==1&&Map[guymapx[whichguy]][guymapy[whichguy]]!=1&&Map[guymapx[whichguy]][guymapy[whichguy]]!=2){guyvelx[whichguy]=.1;}}
		
		if (activity[whichguy]==grenshootright&&anim[whichguy]==44&&targetframe[whichguy]==0&&frame[whichguy]>1){activity[whichguy]=grenidleright;}
		if (activity[whichguy]==grenshootleft&&anim[whichguy]==44&&targetframe[whichguy]==0&&frame[whichguy]>1){activity[whichguy]=grenidleleft;}
		...
		if (activity[whichguy]==swordrightthrust&&anim[whichguy]==27&&targetframe[whichguy]==3){
			...
			if(b!=-1&&activity[b]!=swordleftparry){
				...
				health[b]-=53;
				DoStabFX(whichguy, b);
				if(itemtype[whichguy]==0){playsound(8,80);}
				if(itemtype[whichguy]==1){playsound(12,80);}
				if(health[b]>0){
					whichhurt=0;
					if(health[b]>0){if(activity[b]!=painoneright&&activity[b]!=paintworight&&activity[b]!=painthreeright){targetframe[b]=0; frame[b]=0;} if(activity[b]==painoneright||activity[b]==paintworight||activity[b]==painthreeright){guyx[b]+=2; if(frame[b]>2){targetframe[b]=2; frame[b]=2;}} if(whichhurt==1){activity[b]=painoneright; anim[b]=22; targetanim[b]=22;} if(activity[b]!=painoneright){activity[b]=paintworight; anim[b]=23; targetanim[b]=23;} if(facing[b]==1){activity[b]=painthreeright; anim[b]=42; targetanim[b]=42;} if(Walls[e][f]){activity[b]=hitwallright; guyx[b]=e*10-590; anim[b]=15; targetanim[b]=15;} time[b]=100;}
				}
				if(health[b]<=0){
					activity[b]=fallbackwardsright; 
					targetframe[b]=0;
					frame[b]=0;
					anim[b]=13; targetanim[b]=13;
					if(facing[b]==1){activity[b]=fallforwardsright; anim[b]=41; targetanim[b]=41;}
					if(Walls[e][f]){activity[b]=hitwallright; guyx[b]=e*10-590; anim[b]=15; targetanim[b]=15;} 
					time[b]=100;
				}
			}
			if(b!=-1&&activity[b]==swordleftparry){
				delaytime[b]=0;
				delaytime[whichguy]=0;
				if(itemtype[b]==0){playsound(9,80);}
				if(itemtype[b]==1){playsound(12,80);}
				for(d=0;d<4;d++){
		...
		if (activity[whichguy]==swordleftthrust&&anim[whichguy]==27&&targetframe[whichguy]==3){
			...
			if(b!=-1&&activity[b]!=swordrightparry){
				...
				if(health[b]>0){
					whichhurt=0;
					if(health[b]>0){if(activity[b]!=painoneleft&&activity[b]!=paintwoleft&&activity[b]!=painthreeleft){targetframe[b]=0; frame[b]=0;} if(activity[b]==painoneleft||activity[b]==paintwoleft||activity[b]==painthreeleft){guyx[b]-=2; if(frame[b]>2){targetframe[b]=2; frame[b]=2;}} if(whichhurt==1){activity[b]=painoneleft; anim[b]=22; targetanim[b]=22;} if(activity[b]!=painoneleft){activity[b]=paintwoleft; anim[b]=23; targetanim[b]=23;} if(facing[b]==0){activity[b]=painthreeleft; anim[b]=42; targetanim[b]=42;} if(Walls[e-1][f]){activity[b]=hitwallleft; guyx[b]=e*10-590; anim[b]=15; targetanim[b]=15;} time[b]=100;}
				}
				if(health[b]<=0){
					...
					frame[b]=0;
					anim[b]=13; targetanim[b]=13;
					if(facing[b]==0){activity[b]=fallforwardsleft; anim[b]=41; targetanim[b]=41;}
					if(Walls[e-1][f]){activity[b]=hitwallleft; guyx[b]=e*10-590; anim[b]=15; targetanim[b]=15;} 
					time[b]=100;
				}
			}
			if(b!=-1&&activity[b]==swordrightparry){
		...
		if (activity[whichguy]==shootleft&&targetframe[whichguy]==3&&Walls[guymapx[whichguy]][guymapy[whichguy]]==0&&Walls[c][guymapy[whichguy]]==0){
			...
			if (freezetime==0&&itemnum[whichguy]==1){
				...
				ammo[whichguy]--;
				litx[1]=point.x; lity[1]=point.y; lighton[1]=1; lightbrightness[1]=255;
				if(targetanim[whichguy]==30){guyvelx[whichguy]-=.2;}
			}
			if (freezetime==0&&itemnum[whichguy]==4){
				g=RangedRandom(14,16)+1;
				//display=g;
				if(smokingbody[whichguy][kRightGun]<40){smokingbody[whichguy][kRightGun]+=20;}
				playsound(g,200);
				...
				ammo[whichguy]--;
				litx[0]=point.x; lity[0]=point.y; lighton[0]=1; lightbrightness[0]=255;
				if(targetanim[whichguy]==30){guyvelx[whichguy]-=.05;}
			}
			if (freezetime==0&&itemnum[whichguy]==7){
				while(lightningdelay[whichguy]<=0){
				if(firstshot[whichguy]==1){firstshot[whichguy]=0; lightningdelay[whichguy]=0;}
				...
				point=FindRightGunPoint(whichguy);
				if(g==0){point2=FindBodyPart(b,kHead); smokingbody[b][kHead]=200;}
				if(g==1){point2=FindBodyPart(b,kTorso); smokingbody[b][kTorso]=200;}
				if(g==2){point2=FindBodyPart(b,kRightLowerArm); smokingbody[b][kRightLowerArm]=200;}
				if(g!=0&&g!=1&&g!=2){point2=FindBodyPart(b,kLeftLowerArm); smokingbody[b][kLeftLowerArm]=200;}
				if(lightningfxdelay[whichguy]<5){MakeSprite(point.x, point.y, point.z, 255, lightningflashstars, RangedRandom(0,360), 10, RangedRandom(0,360), 0,0,0); MakeLineLightning(100,0,point.x,point.y,point.z,point2.x,point2.y,point2.z);}
				
				if(lightningfxdelay[whichguy]<5){MakeSprite(point2.x, point2.y, point2.z, 255, lightningflashstars, RangedRandom(0,360), 14, RangedRandom(0,360), 0,0,0);}
				
				f=guyy[b]/-20-.5+40;
				e=(guyx[b]+595-.1)/10;
				usingjetpack[b]=0;
				if(activity[b]!=fallbackwardsright){activity[b]=fallbackwardsright; targetframe[b]=0; frame[b]=0; anim[b]=13; targetanim[b]=13; if(Walls[e][f]){activity[b]=hitwallright; guyx[b]=e*10-590; anim[b]=15; targetanim[b]=15;} time[b]=100;}
				if(facing[b]==1){activity[b]=fallforwardsright; anim[b]=41; targetanim[b]=41;}
				if(Walls[e][f]){activity[b]=hitwallright; guyx[b]=e*10-590; anim[b]=15; targetanim[b]=15;}
				if(Walls[e][f]==0){
					guyvelx[b]=.2;
				...
				if(b!=-1){playsound(g,100);}
				if(b==-1)
				{
					if(lightningfxdelay[whichguy]<5){MakeSprite(point.x, point.y, point.z, 255, lightningflashstars, RangedRandom(0,360), 3, RangedRandom(0,360), 0,0,0); MakeLineLightning(100,0,point.x,point.y,point.z,point.x+RangedRandom(0,1),point.y+RangedRandom(-1,1),point.z+RangedRandom(-1,1));}
					ammo[whichguy]-=.04;
		...
		if (activity[whichguy]==shootright&&targetframe[whichguy]==3&&Walls[guymapx[whichguy]-1][guymapy[whichguy]]==0&&Walls[d-2][guymapy[whichguy]]==0){
			...
			if (freezetime==0&&itemnum[whichguy]==7){
				...
				if(g==2){point2=FindBodyPart(b,kRightLowerArm); smokingbody[b][kRightLowerArm]=200;}
				if(g!=0&&g!=1&&g!=2){point2=FindBodyPart(b,kLeftLowerArm); smokingbody[b][kLeftLowerArm]=200;}
				if(lightningfxdelay[whichguy]<5){MakeLineLightning(100,0,point.x,point.y,point.z,point2.x,point2.y,point2.z);}
				...
				e=(guyx[b]+595-.1)/10;
				usingjetpack[b]=0;
				activity[b]=fallbackwardsleft; targetframe[b]=0; frame[b]=0; anim[b]=13; targetanim[b]=13; if(Walls[e][f]){activity[b]=hitwallleft; guyx[b]=e*10-590; anim[b]=15; targetanim[b]=15;} time[b]=100;
				if(facing[b]==0){activity[b]=fallforwardsleft; anim[b]=41; targetanim[b]=41;}
				if(Walls[e-1][f]){activity[b]=hitwallleft; guyx[b]=e*10-590; anim[b]=15; targetanim[b]=15;}
				if(Walls[e-1][f]==0){
					guyvelx[b]=-.2;
			...
			if (freezetime==0&&itemnum[whichguy]==5&&anim[whichguy]!=30){
				...
				ammo[whichguy]--;
				litx[0]=point.x; lity[0]=point.y+1; lighton[0]=1; lightbrightness[0]=255;
				if(targetanim[whichguy]==30){guyvelx[whichguy]+=.05;}
			}
			if (freezetime==0&&itemnum[whichguy]==5&&anim[whichguy]==30){
				if(smokingbody[whichguy][kRightGun]<40){smokingbody[whichguy][kRightGun]+=20;}
		...
		if(activity[whichguy]==hitwallright||activity[whichguy]==hitwallleft){
			if(frame[whichguy]==9&&Map[guymapx[whichguy]][guymapy[whichguy]]!=0&&Map[guymapx[whichguy]][guymapy[whichguy]]!=3){
				playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);
			}
		}
		if(activity[whichguy]==hitwallright||activity[whichguy]==hitwallleft){
			if(frame[whichguy]==4){
		...
		if(activity[whichguy]==brokeneckleft||activity[whichguy]==brokeneckright){
			if(frame[whichguy]==8){
				if(Map[guymapx[whichguy]][guymapy[whichguy]]!=0&&Map[guymapx[whichguy]][guymapy[whichguy]]!=0){playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);}
			}
			if(frame[whichguy]==4){
				playsound(7,60);
				health[whichguy]-=120;
		...
		if(activity[whichguy]==impaledleft||activity[whichguy]==impaledright){
			...
			if(frame[whichguy]==2){DoStabFX(0,whichguy);}
		}
		...
		if (activity[whichguy]==swordthrowleft||activity[whichguy]==swordthrowright){
			...
			if(frame[whichguy]==4){
				if(itemtype[whichguy]==0){playsound(9,80);}
				if(itemtype[whichguy]==1){playsound(12,80);}
			}
		}
		if (activity[whichguy]==swordthrownleft||activity[whichguy]==swordthrownright){
			...
			if(frame[whichguy]==3||frame[whichguy]==4){DoStabFX(0,whichguy);}
		}
		if (activity[whichguy]==impaleleft||activity[whichguy]==impaleright){
...
void 	NextFrame(int whichguy)
{
			...
			if (time[whichguy]>=100){SwitchFrame(whichguy);}
			if (time[whichguy]>=100){SwitchFrame(whichguy);}
			if (time[whichguy]>=100){SwitchFrame(whichguy);}
			if (time[whichguy]>=100){SwitchFrame(whichguy);}
			if (time[whichguy]>=100){SwitchFrame(whichguy);}
...
void 	CheckWallHit(int a, int b, int c, int x){
	...
	if(sprite[x].velx>0){
		for(f=a;f<=e;f++){
			if(Walls[f][b]==1&&sprite[x].type==lasersprite){
				sprite[x].type=0; sprite[x].velx=0; sprite[x].vely=0; sprite[x].x= f*10-590+2; playsound(6,200);
				for(d=0;d<6;d++){
	...
	if(sprite[x].velx<0){
		for(f=e;f<=a;f++){
			if(Walls[f][b]==1&&sprite[x].type==lasersprite){
				sprite[x].type=0; sprite[x].velx=0; sprite[x].vely=0; sprite[x].x= f*10-590+6; playsound(6,200);
				for(d=0;d<6;d++){
					MakeSprite(sprite[x].x, sprite[x].y, sprite[x].z, 255, wallsparks, 0, 2, 0, RangedRandom(10,0)-10 ,RangedRandom(10,-10)-10,RangedRandom(10,-10)-10);
				}
			}
	...
	if(sprite[x].velx>0){
		for(f=a;f<e;f++){
			...
			if(Walls[f][b]==1&&sprite[x].type==grenade){
				sprite[x].velx*=-1;
				sprite[x].x=f*10-590+1;
				if(sprite[x].stage==1){sprite[x].stage=3;}
				if(sprite[x].stage==0||sprite[x].stage==2){sprite[x].stage=1;}
				if(sprite[x].stage==3){sprite[x].stage=2;}
				if(sprite[x].type2!=0){playsound(13,20+absolute(sprite[x].velx)*10);}
				if(sprite[x].type2==0){Explode(x);}
			}
...
void Explode(int which){
	...
	if(Map[a][b]==1||Map[a][b]==2){
		...
		for(e=0;e<RangedRandom(6,9);e++){
			...
			if(b==64){sprite[unusedsprite].vely=absolute(sprite[unusedsprite].vely);}
			sprite[unusedsprite].velz=(1+(sprite[unusedsprite].z-sprite[which].z))/4;
		}
	}
	if(Walls[a][b]==1){
		if(a!=2&&a!=3&&a!=4&&a!=94){Walls[a][b]=0;}
		for(e=0;e<RangedRandom(6,9);e++){
			...
			sprite[unusedsprite].velx=(1+(sprite[unusedsprite].x-sprite[which].x))*4;
			if(a==94){sprite[unusedsprite].velx=0-absolute(sprite[unusedsprite].velx);}
			if(a==94){sprite[unusedsprite].x-=5;}
			sprite[unusedsprite].vely=(1+(sprite[unusedsprite].y-sprite[which].y))*4;
			sprite[unusedsprite].velz=(1+(sprite[unusedsprite].z-sprite[which].z))/4;
		}
	}
	if(Walls[a-1][b]==1){
		if(a!=2&&a!=3&&a!=4&&a!=94){Walls[a-1][b]=0;}
		for(e=0;e<RangedRandom(6,9);e++){
			...
			sprite[unusedsprite].velx=(1+(sprite[unusedsprite].x-sprite[which].x))*4;
			if(a==3){sprite[unusedsprite].velx=absolute(sprite[unusedsprite].velx);}
			sprite[unusedsprite].vely=(1+(sprite[unusedsprite].y-sprite[which].y))*4;
			sprite[unusedsprite].velz=(1+(sprite[unusedsprite].z-sprite[which].z))/4;
		}
	}
	...
	for(y=0;y<numplayers;y++){
		...
		if(guyx[y]>=sprite[which].x){
			usingjetpack[y]=0;
			activity[y]=fallbackwardsright; targetframe[y]=0; frame[y]=0; anim[y]=13; targetanim[y]=13; if(Walls[e][f]){activity[y]=hitwallright; guyx[y]=e*10-590; anim[y]=15; targetanim[y]=15;} time[y]=100;
			if(facing[y]==1){activity[y]=fallforwardsright; anim[y]=41; targetanim[y]=41;}
			if(Walls[e][f]){activity[y]=hitwallright; guyx[y]=e*10-590; anim[y]=15; targetanim[y]=15;}
		}
		if(guyx[y]<sprite[which].x){
			usingjetpack[y]=0;
			activity[y]=fallbackwardsleft; targetframe[y]=0; frame[y]=0; anim[y]=13; targetanim[y]=13; if(Walls[e][f]){activity[y]=hitwallleft; guyx[y]=e*10-590; anim[y]=15; targetanim[y]=15;} time[y]=100;
			if(facing[y]==0){activity[y]=fallforwardsleft; anim[y]=41; targetanim[y]=41;}
			if(Walls[e-1][f]){activity[y]=hitwallleft; guyx[y]=e*10-590; anim[y]=15; targetanim[y]=15;}
		}
...
void HandleSprites(){
	...
	for (x=0;x<maxsprites;x++){
		...
		if(sprite[x].type==grenade&&sprite[x].delay<0&&sprite[x].life<10000){
			while(sprite[x].delay<0){sprite[x].delay+=10;}
		...
		if(sprite[x].type==exploflame){
			if(absolute(sprite[x].velx)<multiplier/10){sprite[x].velx=0;}
			if(sprite[x].velx>0){sprite[x].velx-=multiplier/10;}
			if(sprite[x].velx<0){sprite[x].velx+=multiplier/10;}
			if(absolute(sprite[x].vely)<multiplier/10){sprite[x].vely=0;}
			if(sprite[x].vely>0){sprite[x].vely-=multiplier/10;}
			if(sprite[x].vely<0){sprite[x].vely+=multiplier/10;}
			if(absolute(sprite[x].velz)<multiplier/10){sprite[x].velz=0;}
			if(sprite[x].velz>0){sprite[x].velz-=multiplier/10;}
			if(sprite[x].velz<0){sprite[x].velz+=multiplier/10;}
		}
		/*a=(sprite[x].x+595)/10;
		b=sprite[x].y/-20+40;
		...
		if(c!=b&&Map[a][b]!=3&&Map[a][b]!=0&&sprite[x].vely<0&&c<99){
			...
			if(sprite[x].vely<=2){
				for(f=0;f<10;f++){
					if(sprite[x].spin<0){sprite[x].spin+=360;}
					if(sprite[x].spin>=360){sprite[x].spin-=360;}
				}
				sprite[x].vely=0; sprite[x].velx=0; sprite[x].velz=0; sprite[x].y-=.5;
				if(sprite[x].spin<90||sprite[x].spin>270){sprite[x].spin=0;} 
				if(sprite[x].spin>90&&sprite[x].spin<270){sprite[x].spin=180;}}
			if(sprite[x].type==grenade){
				if(sprite[x].stage==1){sprite[x].stage=3;}
				if(sprite[x].stage==0||sprite[x].stage==2){sprite[x].stage=1;}
				if(sprite[x].stage==3){sprite[x].stage=2;}
				if(sprite[x].type2==1){playsound(13,20+sprite[x].vely*10);}
				if(sprite[x].type2==0){Explode(x);}
			}
			if(sprite[x].type==chunk){
		...
		if(Map[a][d]==3||Map[a][d]==0){
			if(sprite[x].vely==0){sprite[x].vely=0.00001;}
		}
		}
		if(sprite[x].type==lasersprite||sprite[x].type==tracersprite||sprite[x].type==water||sprite[x].type==grenade||sprite[x].type==chunk){
			...
			if(sprite[x].type!=water){
				if(sprite[x].type!=grenade||sprite[x].stage==0){
					...
					if(sprite[x].type==grenade&&sprite[x].velx<0){sprite[x].spin+=180;}
				}
			}
			...
			if(sprite[x].velx>0&&sprite[x].type!=chunk&&sprite[x].type!=grenade){sprite[x].x+=2;}
			if(sprite[x].velx<0&&sprite[x].type!=chunk&&sprite[x].type!=grenade){sprite[x].x-=2;}
			for(y=0;y<numplayers;y++){
				abouttobehit[y]=0;
				e=(guyx[y]+595)/10;
				...
				wallintheway=0;
				if(Walls[e-1][f]==1&&sprite[x].velx>0&&e>a){wallintheway=1;}
				if(Walls[e][f]==1&&sprite[x].velx<0&&e<a){wallintheway=1;}
				e=(guyx[y]+595-.1)/10;
				if(Walls[e-1][f]==1&&sprite[x].velx>0&&e>a){wallintheway=1;}
				if(Walls[e][f]==1&&sprite[x].velx<0&&e<a){wallintheway=1;}
				a=(sprite[x].x+595)/10;
				...
				if (activity[y]!=ducking&&activity[y]!=leftroll&&activity[y]!=rightroll&&activity[y]!=splat&&wallintheway==0){
					if (a>c&&sprite[x].x+absolute(guyvelx[y])+(sprite[x].velx*multiplier/100)>guyx[y]-5&&sprite[x].x<guyx[y]-5&&absolute(sprite[x].y-guyy[y]-7)<7*size[y]&&activity[y]!=fallforwardsleft&&activity[y]!=fallforwardsright&&activity[y]!=fallbackwardsleft&&activity[y]!=fallbackwardsright&&activity[y]!=landonback){
						...
						if(sprite[x].type==tracersprite){
							health[y]-=23;
							if(sprite[x].stage==1){health[y]-=15;}
						}
						if(health[y]>0&&activity[y]!=climbrope&&activity[y]!=onrope&&activity[y]!=downrope&&activity[y]!=falling&&activity[y]!=leftflip&&activity[y]!=rightflip){
							...
							if(health[y]>0){if(activity[y]!=painoneright&&activity[y]!=paintworight&&activity[y]!=painthreeright){targetframe[y]=0; frame[y]=0;} if(activity[y]==painoneright||activity[y]==paintworight||activity[y]==painthreeright){guyx[y]+=2; if(frame[y]>2){targetframe[y]=2; frame[y]=2;}} if(whichhurt==1){activity[y]=painoneright; anim[y]=22; targetanim[y]=22;} if(activity[y]!=painoneright){activity[y]=paintworight; anim[y]=23; targetanim[y]=23;} if(facing[y]==1){activity[y]=painthreeright; anim[y]=42; targetanim[y]=42;} if(Walls[e][f]){activity[y]=hitwallright; guyx[y]=e*10-590; anim[y]=15; targetanim[y]=15;} time[y]=100;}
						}
						if(sprite[x].type==chunk){sprite[x].velx*=-1;sprite[x].x=guyx[y]; health[y]-=37;}
						...
						for(d=0;d<4;d++){
							if(sprite[x].type==tracersprite){MakeSprite(guyx[y], sprite[x].y, sprite[x].z, 255, bloodsparks, 0, 2, 0, RangedRandom(5,-5)-5 ,RangedRandom(5,-5)-5,RangedRandom(5,-5)-5);}
							if(sprite[x].type==lasersprite){MakeSprite(guyx[y], sprite[x].y, sprite[x].z, 255, lasersparks, 0, 2, 0, RangedRandom(5,-5)-5 ,RangedRandom(5,-5)-5,RangedRandom(5,-5)-5);}
						}
						if(sprite[x].type==grenade){sprite[x].velx*=-1;sprite[x].x=guyx[y]-.1; if(sprite[x].type2==0){Explode(x);}}
						if(sprite[x].type!=grenade&&sprite[x].type!=chunk){sprite[x].type=0; sprite[x].velx=0; sprite[x].x=guyx[y]; sprite[x].vely=0;}
						playsound(6,200);
						}
						if(activity[y]==swordleftparry&&frame[y]<=3&&itemtype[y]==1){
							for(d=0;d<3;d++){
							...
							if(itemtype[y]==0){playsound(8,80);}
							if(itemtype[y]==1){playsound(12,80);}
							sprite[x].velx*=-1;
						}
					}
					if (a<c&&sprite[x].x-absolute(guyvelx[y])+(sprite[x].velx*multiplier/100)<guyx[y]+5&&sprite[x].x>guyx[y]+5&&absolute(sprite[x].y-guyy[y]-7)<7*size[y]&&activity[y]!=fallbackwardsleft&&activity[y]!=fallbackwardsright&&activity[y]!=fallforwardsleft&&activity[y]!=fallforwardsright&&activity[y]!=landonback){
						...
						if(health[y]>0&&activity[y]!=climbrope&&activity[y]!=onrope&&activity[y]!=downrope&&activity[y]!=falling&&activity[y]!=leftflip&&activity[y]!=rightflip){
							whichhurt=RangedRandom(0,2);
							if(health[y]>0){if(activity[y]!=painoneleft&&activity[y]!=paintwoleft&&activity[y]!=painthreeleft){targetframe[y]=0; frame[y]=0;} if(activity[y]==painoneleft||activity[y]==paintwoleft||activity[y]==painthreeleft){guyx[y]-=2; if(frame[y]>2){targetframe[y]=2; frame[y]=2;}} if(whichhurt==1){activity[y]=painoneleft; anim[y]=22; targetanim[y]=22;} if(activity[y]!=painoneleft){activity[y]=paintwoleft; anim[y]=23; targetanim[y]=23;} if(facing[y]==0){activity[y]=painthreeleft; anim[y]=42; targetanim[y]=42;} if(Walls[e-1][f]){activity[y]=hitwallleft; guyx[y]=e*10-590; anim[y]=15; targetanim[y]=15;} time[y]=100;}
						}
						if(sprite[x].type==chunk){sprite[x].velx*=-1;sprite[x].x=guyx[y]+.1; health[y]-=37;}
						...
						if(sprite[x].type==grenade){if(sprite[x].type2==0){Explode(x);}sprite[x].velx*=-1;sprite[x].x=guyx[y];}
						if(sprite[x].type!=grenade&&sprite[x].type!=chunk){sprite[x].type=0; sprite[x].velx=0; sprite[x].x=guyx[y]; sprite[x].vely=0;}
						playsound(6,200);
						}
						if(activity[y]==swordrightparry&&frame[y]<=3&&itemtype[y]==1){
				...
				if (activity[y]==ducking||activity[y]==leftroll||activity[y]==rightroll||activity[y]==splat){
					if (a>c&&sprite[x].x+absolute(guyvelx[y])+(sprite[x].velx*multiplier/100)>guyx[y]&&sprite[x].x<guyx[y]&&absolute(sprite[x].y-guyy[y]-3)<3*size[y]&&activity[y]!=fallbackwardsleft&&activity[y]!=fallbackwardsright&&activity[y]!=fallforwardsleft&&activity[y]!=fallforwardsright&&activity[y]!=landonback){
						...
						if(sprite[x].type==grenade){if(sprite[x].type2==0){Explode(x);}sprite[x].velx*=-1;sprite[x].x=guyx[y]-.1;}
						if(sprite[x].type!=grenade&&sprite[x].type!=chunk){sprite[x].type=0; sprite[x].velx=0; sprite[x].x=guyx[y]; sprite[x].vely=0;}
						playsound(6,200);
					}
			...
			d=0;
			if(hit==0){
			if(sprite[x].velx>0){sprite[x].x-=4;}
			if(sprite[x].velx<0){sprite[x].x+=4;}
			for(y=0;y<numplayers;y++){
				e=(guyx[y]+595)/10;
				f=guyy[y]/-20-.5+40;
				...
				if (activity[y]!=ducking&&activity[y]!=leftroll&&activity[y]!=rightroll&&activity[y]!=splat&&wallintheway==0){
					if (a>c&&sprite[x].x+guyvelx[y]+(sprite[x].velx*multiplier/100)>guyx[y]-5&&sprite[x].x<guyx[y]-5&&absolute(sprite[x].y-guyy[y]-7)<7*size[y]&&activity[y]!=fallforwardsleft&&activity[y]!=fallforwardsright&&activity[y]!=fallbackwardsleft&&activity[y]!=fallbackwardsright&&activity[y]!=landonback){
						...
						if(sprite[x].type==grenade){sprite[x].velx*=-1;sprite[x].x=guyx[y]; if(sprite[x].type2==0){Explode(x);}}
						if(sprite[x].type!=grenade&&sprite[x].type!=chunk){sprite[x].type=0; sprite[x].velx=0; sprite[x].x=guyx[y]; sprite[x].vely=0;}
						playsound(6,200);
						}
						if(activity[y]==swordleftparry&&frame[y]<=3&&itemtype[y]==1){
		...
		if(sprite[x].type==spawnstars){sprite[x].spin+=multiplier*2;}
		if(sprite[x].type==spawnstars){sprite[x].brightness-=multiplier/2;}
		if(sprite[x].type==deletestars){sprite[x].spin+=multiplier*2;}
		if(sprite[x].type==deletestars){sprite[x].brightness-=multiplier/2;}
		if(sprite[x].type==muzzleflashstars){sprite[x].spin+=multiplier*2;}
		if(sprite[x].type==muzzleflashstars&&sprite[x].fading>=6){sprite[x].brightness-=multiplier*12;}
		if(sprite[x].type==muzzleflashstars){sprite[x].fading++;}
		if(sprite[x].type==lightningflashstars){sprite[x].spin+=multiplier*2;}
		if(sprite[x].type==lightningflashstars&&sprite[x].fading>=6){sprite[x].brightness-=multiplier*4;}
		if(sprite[x].type==lightningflashstars){sprite[x].fading++;}
		if(sprite[x].type==bouncesprite){sprite[x].spin+=multiplier*2;}
		if(sprite[x].type==water){sprite[x].spin+=multiplier*2;}
		if(sprite[x].type==wallsparks){sprite[x].spin+=multiplier*2;}
		if(sprite[x].type==wallsparks){sprite[x].brightness-=multiplier*4;}
		if(sprite[x].type==bloodsparks){sprite[x].spin+=multiplier*2;}
		if(sprite[x].type==bloodsparks){sprite[x].brightness-=multiplier*2;}
		if(sprite[x].type==lasersparks){sprite[x].spin+=multiplier*2;}
		if(sprite[x].type==lasersparks){sprite[x].brightness-=multiplier*2;}
		if(sprite[x].type==invisiblestars){sprite[x].brightness-=multiplier;}
		if(sprite[x].type==invisiblestars2){sprite[x].brightness-=multiplier;}
		if(sprite[x].type==smokesprite&&sprite[x].type2==0){sprite[x].spin+=multiplier*3;}
		if(sprite[x].type==smokesprite&&sprite[x].type2==0){sprite[x].brightness-=multiplier;}
		if(sprite[x].type==smokesprite&&sprite[x].type2==1){sprite[x].spin+=multiplier/4;}
		if(sprite[x].type==smokesprite&&sprite[x].type2==1){sprite[x].brightness-=multiplier/20*2;}
		if(sprite[x].type==exploflame){sprite[x].spin+=multiplier*2;}
		if(sprite[x].type==exploflame){sprite[x].brightness-=multiplier/2;}
		if(sprite[x].type==JetPackGlow){sprite[x].brightness=200;}
		if(sprite[x].brightness<=0){sprite[x].type=0;}
		if(sprite[x].vely!=0&&sprite[x].type!=tracersprite&&sprite[x].type!=smokesprite&&sprite[x].type!=exploflame){sprite[x].vely-=1*multiplier*gravity; if(sprite[x].vely==0){sprite[x].vely+=.00001;}}
...
Point3D FindRightGunPoint(int whichguy){
	...
	glTranslatef(guyx[whichguy],guyy[whichguy],0.0f);
	if (activity[whichguy]==idle1||activity[whichguy]==idle2||activity[whichguy]==climbrope||activity[whichguy]==onrope||activity[whichguy]==downrope){targetrotation[whichguy]=0;}
	if (activity[whichguy]==runleft||activity[whichguy]==leftflip||activity[whichguy]==leftroll||activity[whichguy]==fallbackwardsright||activity[whichguy]==hitwallright||activity[whichguy]==brokeneckleft||activity[whichguy]==breakneckleft||activity[whichguy]==impaleleft||activity[whichguy]==impaledleft||activity[whichguy]==duckimpaleleft||activity[whichguy]==duckimpaledleft||activity[whichguy]==swordthrowleft||activity[whichguy]==swordthrownleft||activity[whichguy]==painoneright||activity[whichguy]==paintworight||activity[whichguy]==swordidleleft||activity[whichguy]==swordleftthrust||activity[whichguy]==jetleft){targetrotation[whichguy]=-90;}
	if (activity[whichguy]==brokeneckleft||activity[whichguy]==breakneckleft||activity[whichguy]==impaleleft||activity[whichguy]==impaledleft||activity[whichguy]==duckimpaleleft||activity[whichguy]==duckimpaledleft||activity[whichguy]==swordthrowleft||activity[whichguy]==swordthrownleft||activity[whichguy]==painoneright||activity[whichguy]==paintworight||activity[whichguy]==swordleftthrust||activity[whichguy]==swordrightparry||activity[whichguy]==jetimpaleleft||activity[whichguy]==jetimpaledleft){rotation[whichguy]=-90; targetrotation[whichguy]=-90;}
	if (activity[whichguy]==runright||activity[whichguy]==rightflip||activity[whichguy]==rightroll||activity[whichguy]==fallbackwardsleft||activity[whichguy]==hitwallleft||activity[whichguy]==brokeneckright||activity[whichguy]==breakneckright||activity[whichguy]==impaleright||activity[whichguy]==impaledright||activity[whichguy]==swordthrowright||activity[whichguy]==swordthrownright||activity[whichguy]==painoneleft||activity[whichguy]==paintwoleft||activity[whichguy]==swordidleright||activity[whichguy]==swordrightthrust||activity[whichguy]==jetright){targetrotation[whichguy]=90;}
	if (activity[whichguy]==brokeneckright||activity[whichguy]==breakneckright||activity[whichguy]==impaleright||activity[whichguy]==impaledright||activity[whichguy]==duckimpaleright||activity[whichguy]==duckimpaledright||activity[whichguy]==swordthrowright||activity[whichguy]==swordthrownright||activity[whichguy]==painoneleft||activity[whichguy]==paintwoleft||activity[whichguy]==swordrightthrust||activity[whichguy]==swordrightparry||activity[whichguy]==jetimpaleright||activity[whichguy]==jetimpaledright){rotation[whichguy]=90; targetrotation[whichguy]=90;}
	if(activity[whichguy]==falling&&guyvelx[whichguy]>0){targetrotation[whichguy]=90;}
	if (activity[whichguy]==falling&&guyvelx[whichguy]<0){targetrotation[whichguy]=-90;}
	if (activity[whichguy]==climbrope||activity[whichguy]==onrope||activity[whichguy]==downrope){glTranslatef(0.0f,0.0f,-1.0f); targetrotation[whichguy]=0; rotation[whichguy]=0;}
	glRotatef(((rotation[whichguy]*(100-time[whichguy]))+(targetrotation[whichguy]*(time[whichguy])))/100,0.0f,1.0f,0.0f);
...
Point3D FindEyeRot(int whichguy){
	...
	if(EyePoint2.z!=EyePoint.z){ Angle.y = atan((EyePoint2.x - EyePoint.x) / (EyePoint2.z - EyePoint.z))/6.28*360+180;}
    ...
    Angle.y*=-1;
    Angle.x=((hiprot[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(hiprot[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100
    +((torsorot[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(torsorot[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100
    +((head[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(head[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100;
...
Point3D FindJetPackPos(int whichguy){
	...
	glTranslatef(guyx[whichguy],guyy[whichguy],0.0f);
	if (activity[whichguy]==idle1||activity[whichguy]==idle2||activity[whichguy]==climbrope||activity[whichguy]==onrope||activity[whichguy]==downrope){targetrotation[whichguy]=0;}
	if (activity[whichguy]==runleft||activity[whichguy]==leftflip||activity[whichguy]==leftroll||activity[whichguy]==fallbackwardsright||activity[whichguy]==hitwallright||activity[whichguy]==brokeneckleft||activity[whichguy]==breakneckleft||activity[whichguy]==impaleleft||activity[whichguy]==impaledleft||activity[whichguy]==duckimpaleleft||activity[whichguy]==duckimpaledleft||activity[whichguy]==swordthrowleft||activity[whichguy]==swordthrownleft||activity[whichguy]==painoneright||activity[whichguy]==paintworight||activity[whichguy]==swordidleleft||activity[whichguy]==grenidleleft||activity[whichguy]==swordleftthrust||activity[whichguy]==jetleft){targetrotation[whichguy]=-90;}
	if (activity[whichguy]==brokeneckleft||activity[whichguy]==breakneckleft||activity[whichguy]==impaleleft||activity[whichguy]==impaledleft||activity[whichguy]==duckimpaleleft||activity[whichguy]==duckimpaledleft||activity[whichguy]==swordthrowleft||activity[whichguy]==swordthrownleft||activity[whichguy]==painoneright||activity[whichguy]==paintworight||activity[whichguy]==swordleftthrust||activity[whichguy]==swordrightparry||activity[whichguy]==jetimpaleleft||activity[whichguy]==jetimpaledleft||activity[whichguy]==grenshootleft){rotation[whichguy]=-90; targetrotation[whichguy]=-90;}
	if (activity[whichguy]==runright||activity[whichguy]==rightflip||activity[whichguy]==rightroll||activity[whichguy]==fallbackwardsleft||activity[whichguy]==hitwallleft||activity[whichguy]==brokeneckright||activity[whichguy]==breakneckright||activity[whichguy]==impaleright||activity[whichguy]==impaledright||activity[whichguy]==swordthrowright||activity[whichguy]==swordthrownright||activity[whichguy]==painoneleft||activity[whichguy]==paintwoleft||activity[whichguy]==swordidleright||activity[whichguy]==grenidleright||activity[whichguy]==swordrightthrust||activity[whichguy]==jetright){targetrotation[whichguy]=90;}
	if (activity[whichguy]==brokeneckright||activity[whichguy]==breakneckright||activity[whichguy]==impaleright||activity[whichguy]==impaledright||activity[whichguy]==duckimpaleright||activity[whichguy]==duckimpaledright||activity[whichguy]==swordthrowright||activity[whichguy]==swordthrownright||activity[whichguy]==painoneleft||activity[whichguy]==paintwoleft||activity[whichguy]==swordrightthrust||activity[whichguy]==swordrightparry||activity[whichguy]==jetimpaleright||activity[whichguy]==jetimpaledright||activity[whichguy]==grenshootright){rotation[whichguy]=90; targetrotation[whichguy]=90;}
	if(activity[whichguy]==falling&&guyvelx[whichguy]>0){targetrotation[whichguy]=90;}
	if (activity[whichguy]==falling&&guyvelx[whichguy]<0){targetrotation[whichguy]=-90;}
	if (activity[whichguy]==climbrope||activity[whichguy]==onrope||activity[whichguy]==downrope){glTranslatef(0.0f,0.0f,-1.0f); targetrotation[whichguy]=0; rotation[whichguy]=0;}
	glRotatef(((rotation[whichguy]*(100-time[whichguy]))+(targetrotation[whichguy]*(time[whichguy])))/100,0.0f,1.0f,0.0f);
...
Point3D FindBodyPart(int whichguy, int whichpart)
{
	...
	glPushMatrix();
		glRotatef(((hiprot[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(hiprot[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((hiprot[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(hiprot[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((hiprot[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(hiprot[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		glTranslatef(((hipfloat[1][0]*(100-time[whichguy]))+(hipfloat[1][1]*(time[whichguy])))/100,((hipfloat[2][0]*(100-time[whichguy]))+(hipfloat[2][1]*(time[whichguy])))/100,((hipfloat[0][0]*(100-time[whichguy]))+(hipfloat[0][1]*(time[whichguy])))/100);
		if(whichpart==kHip){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
		glTranslatef(0.0f,0.5f,0.0f);
		glRotatef(((torsorot[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(torsorot[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((torsorot[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(torsorot[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f);glRotatef(((torsorot[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(torsorot[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		if(whichpart==kTorso){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
		glPushMatrix();
			glTranslatef(0.0f,3.5f,0.0f);
			if(whichpart==kHead){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
		glPopMatrix();
		glPushMatrix();
			glTranslatef(1.5f,3.0f,0.0f);
			glScalef(.8,.8,.8);
			glRotatef(((lefthigharm[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(lefthigharm[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((lefthigharm[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(lefthigharm[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((lefthigharm[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(lefthigharm[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
			if(whichpart==kLeftUpperArm){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
			glTranslatef(0.0f,-2.5f,0.0f);
			glRotatef(((leftlowarm[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftlowarm[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((leftlowarm[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftlowarm[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((leftlowarm[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftlowarm[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
			if(whichpart==kLeftLowerArm){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
			glTranslatef(0.0f,-1.8f,0.0f);
			glRotatef(90,1.0,0.0,0.0);
			glTranslatef(-0.3f,-1.0f,0.8f);
			if(whichpart==kLeftHand){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
		glPopMatrix();
		glPushMatrix();
			glTranslatef(-1.5f,3.0f,0.0f);
			glScalef(-.8,.8,.8);
			glRotatef(((righthigharm[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthigharm[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((righthigharm[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthigharm[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((righthigharm[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthigharm[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
			if(whichpart==kRightUpperArm){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
			glTranslatef(0.0f,-2.5f,0.0f);
			glRotatef(((rightlowarm[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightlowarm[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((rightlowarm[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightlowarm[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((rightlowarm[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightlowarm[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
			if(whichpart==kRightLowerArm){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
			glTranslatef(0.0f,-1.8f,0.0f);
			glRotatef(((righthand[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthand[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((righthand[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthand[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((righthand[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthand[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
			glRotatef(90,1.0,0.0,0.0);
			glTranslatef(-0.3f,-1.0f,0.8f);
			if(whichpart==kRightHand){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
		glPopMatrix();
		glRotatef(-torsorot[0][frame[whichguy]][anim[whichguy]],1.0f,0.0f,0.0f); glRotatef(-torsorot[1][frame[whichguy]][anim[whichguy]],0.0f,1.0f,0.0f); glRotatef(-torsorot[2][frame[whichguy]][anim[whichguy]],0.0f,0.0f,1.0f);
		glTranslatef(0.0f,-0.5f,0.0f);
	glPopMatrix();
	glPushMatrix();
		glTranslatef(0.8f,0.0f,0.0f);
		glRotatef(((lefthighleg[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(lefthighleg[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((lefthighleg[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(lefthighleg[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((lefthighleg[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(lefthighleg[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		if(whichpart==kLeftUpperLeg){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
		glTranslatef(0.0f,-2.5f,0.0f);
		glRotatef(((leftlowleg[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftlowleg[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((leftlowleg[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftlowleg[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((leftlowleg[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftlowleg[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		if(whichpart==kLeftLowerLeg){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
		glTranslatef(0.0f,-1.5f,0.0f);
		glRotatef(((leftfoot[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftfoot[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((leftfoot[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftfoot[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f);glRotatef(((leftfoot[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(leftfoot[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		if(whichpart==kLeftFoot){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
	glPopMatrix();
	glPushMatrix();
		glTranslatef(-0.8f,0.0f,0.0f);
		glRotatef(((righthighleg[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthighleg[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((righthighleg[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthighleg[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((righthighleg[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(righthighleg[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		if(whichpart==kRightUpperLeg){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
		glTranslatef(0.0f,-2.5f,0.0f);
		glRotatef(((rightlowleg[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightlowleg[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((rightlowleg[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightlowleg[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f); glRotatef(((rightlowleg[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightlowleg[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		if(whichpart==kRightLowerLeg){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
		glTranslatef(0.0f,-1.5f,0.0f);
		glRotatef(((rightfoot[0][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightfoot[0][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,1.0f,0.0f,0.0f); glRotatef(((rightfoot[1][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightfoot[1][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,1.0f,0.0f);glRotatef(((rightfoot[2][frame[whichguy]][anim[whichguy]]*(100-time[whichguy]))+(rightfoot[2][targetframe[whichguy]][targetanim[whichguy]]*(time[whichguy])))/100,0.0f,0.0f,1.0f);
		if(whichpart==kRightFoot){glGetFloatv(GL_MODELVIEW_MATRIX,M);}
	glPopMatrix();
	glPopMatrix();
	glPopMatrix();
	GunPoint.x=M[12];
	GunPoint.y=M[13];
	GunPoint.z=M[14];
	if(whichpart==kRightGun){GunPoint=FindRightGunPoint(whichguy);}
	return GunPoint;
}
/*
Point3D FindJetPackPos(int whichguy){
...
void DoAIPlayerStuff(int whichguy){
	...
	if(ammo[whichguy]<0){ammo[whichguy]=0;}
	for(x=0;x<maxswordtrail;x++){
		SwordTrail[whichguy][0].brightness[x][3]-=multiplier/30;
		if(SwordTrail[whichguy][0].brightness[x][3]<0){SwordTrail[whichguy][0].brightness[x][3]=0;}
	}
	for(x=0;x<maxswordtrail;x++){
		SwordTrail[whichguy][1].brightness[x][3]-=multiplier/30;
		if(SwordTrail[whichguy][1].brightness[x][3]<0){SwordTrail[whichguy][1].brightness[x][3]=0;}
	}
	...
	if(frame[whichguy]==2){lightningdelay[whichguy]-=multiplier;}
	delaytime[whichguy]-=multiplier;
	int guywillbe;
		oldguyx[whichguy]=guyx[whichguy]; oldguyy[whichguy]=guyy[whichguy];
		if (cloaktime[whichguy]<100){
			cloaktime[whichguy]+=multiplier;
			if (cloaktime[whichguy]>=100&&invisible[whichguy]==1){invisible[whichguy]=2;}
			if (cloaktime[whichguy]>=100&&invisible[whichguy]==0){invisible[whichguy]=1;}
			if (invisible[whichguy]==2){invisible[whichguy]=0;}
		}
		if (cloaktime[whichguy]>100){
			cloaktime[whichguy]+=multiplier;
		}
		if (activity[whichguy]==deadfacedown&&dead[whichguy]==1&&health[whichguy]<=0){verydead=1;}
		if (activity[whichguy]==deadfaceup&&dead[whichguy]==1&&health[whichguy]<=0){verydead=1;}
		if(dead[whichguy]==1&&health[whichguy]<=0&&invisible[whichguy]==1&&cloaktime[whichguy]>=100){cloaktime[whichguy]=0;}
		if(whichguy==0){verydeadshow=verydead;}
		if(verydead==0&&activity[whichguy]!=onrope&&activity[whichguy]!=downrope){
		oldtime[whichguy]=time[whichguy];
		if (anim[whichguy]==7||anim[whichguy]==8){time[whichguy] +=(((float)speed[frame[whichguy]][anim[whichguy]]/1000))*multiplier/3;}
		if (anim[whichguy]==7){time[whichguy] +=(((float)speed[frame[whichguy]][anim[whichguy]]/1000))*multiplier/2;}
		if (time[whichguy]<100&&timedirection[whichguy]==1){time[whichguy] +=(((float)speed[frame[whichguy]][anim[whichguy]]/1000))*multiplier/3;}
		if (time[whichguy]<100&&timedirection[whichguy]==-1){time[whichguy] +=((float)speed[targetframe[whichguy]][targetanim[whichguy]]/1000);}
		NextFrame(whichguy);
		}
		...
		if (activity[whichguy]!=climbrope&&activity[whichguy]!=onrope&&activity[whichguy]!=downrope){
			if (activity[whichguy]==idle1){targetanim[whichguy]=0;}
			if (activity[whichguy]==idle2){targetanim[whichguy]=1;}
			if (activity[whichguy]==runleft&&CheckAIKey( whichguy, kLeftKey[whichguy] )==0){activity[whichguy]=RangedRandom(1,2); time[whichguy]=0;}
			if (activity[whichguy]==runright&&CheckAIKey( whichguy, kRightKey[whichguy] )==0){activity[whichguy]=RangedRandom(1,2); time[whichguy]=0;}
			if (activity[whichguy]==ducking&&CheckAIKey( whichguy, kDownKey[whichguy] )==0){activity[whichguy]=RangedRandom(1,2); time[whichguy]=0;}
			if (activity[whichguy]==runleft){targetanim[whichguy]=2; guyx[whichguy]-=.6*multiplier/5*size[whichguy];}
			if (activity[whichguy]==runright){targetanim[whichguy]=2; guyx[whichguy]+=.6*multiplier/5*size[whichguy];}
			if (activity[whichguy]==swordleftspin&&frame[whichguy]<=2){guyx[whichguy]-=.3*multiplier/5*size[whichguy];}
			if (activity[whichguy]==swordrightspin&&frame[whichguy]<=2){guyx[whichguy]+=.3*multiplier/5*size[whichguy];}
			if (activity[whichguy]==leftroll&&guyvely[whichguy]==0&&guyvelx[whichguy]==0){targetanim[whichguy]=8; guyx[whichguy]-=.6*multiplier/5*size[whichguy]; }
		if (activity[whichguy]==rightroll&&guyvely[whichguy]==0&&guyvelx[whichguy]==0){targetanim[whichguy]=8; guyx[whichguy]+=.6*multiplier/5*size[whichguy];}
			if (activity[whichguy]==ducking){targetanim[whichguy]=3;}
			if (activity[whichguy]==falling){targetanim[whichguy]=4;}
		}
		
		if(activity[whichguy]==idle1||activity[whichguy]==idle2){
			if(itemnum[whichguy]==2||itemnum[whichguy]==3){
				if(facing[whichguy]==1){activity[whichguy]=swordidleright;}
				if(facing[whichguy]!=1){activity[whichguy]=swordidleleft;}
				targetanim[whichguy]=26;
			}
			if(itemnum[whichguy]==6){
				if(facing[whichguy]==1){activity[whichguy]=grenidleright;}
				if(facing[whichguy]!=1){activity[whichguy]=grenidleleft;}
		...
		if(activity[whichguy]==jetidle){
			if(CheckAIKey( whichguy, kDownKey[whichguy] )==0){
				if(absolute(guyvely[whichguy])<multiplier/slowspeed){guyvely[whichguy]=0;}
				if(guyvely[whichguy]<0){guyvely[whichguy]+=multiplier/slowspeed;}
				if(guyvely[whichguy]>0){guyvely[whichguy]-=multiplier/slowspeed;}
			}
			if(absolute(guyvelx[whichguy])<multiplier/slowspeed){guyvelx[whichguy]=0;}
			if(guyvelx[whichguy]<0){guyvelx[whichguy]+=multiplier/slowspeed;}
			if(guyvelx[whichguy]>0){guyvelx[whichguy]-=multiplier/slowspeed;}
		}
		if(holding[whichguy]!=0){holding[whichguy]+=multiplier/10;}
		if(activity[whichguy]==fallforwardsleft&&targetframe[whichguy]<=5&&guyvelx[whichguy]==0){guyx[whichguy]-=.6*multiplier/5*size[whichguy];}
		if(activity[whichguy]==fallforwardsright&&targetframe[whichguy]<=5&&guyvelx[whichguy]==0){guyx[whichguy]+=.6*multiplier/5*size[whichguy];}
		if(activity[whichguy]==jetright){guyvelx[whichguy]=(.4+guyvelx[whichguy]/multiplier*100)/(1/multiplier*100+1); if(CheckAIKey( whichguy, kUpKey[whichguy] )==0&&CheckAIKey( whichguy, kDownKey[whichguy] )==0){if(absolute(guyvely[whichguy])<multiplier/slowspeed){guyvely[whichguy]=0;}if(guyvely[whichguy]<0){guyvely[whichguy]+=multiplier/slowspeed;} if(guyvely[whichguy]>0){guyvely[whichguy]-=multiplier/slowspeed;}}}
		if(activity[whichguy]==jetleft){guyvelx[whichguy]=(-.4+guyvelx[whichguy]/multiplier*100)/(1/multiplier*100+1); if(CheckAIKey( whichguy, kUpKey[whichguy] )==0&&CheckAIKey( whichguy, kDownKey[whichguy] )==0){if(absolute(guyvely[whichguy])<multiplier/slowspeed){guyvely[whichguy]=0;}if(guyvely[whichguy]<0){guyvely[whichguy]+=multiplier/slowspeed;} if(guyvely[whichguy]>0){guyvely[whichguy]-=multiplier/slowspeed;}}}
		if(activity[whichguy]==jetup||activity[whichguy]==jetright||activity[whichguy]==jetleft||activity[whichguy]==jetidle){if(CheckAIKey( whichguy, kUpKey[whichguy] )==1){guyvely[whichguy]=(.4+guyvely[whichguy]/multiplier*100)/(1/multiplier*100+1);}}
		if(activity[whichguy]==jetup){if(absolute(guyvely[whichguy])<multiplier/slowspeed){guyvelx[whichguy]=0;}if(guyvelx[whichguy]<0){guyvelx[whichguy]+=multiplier/slowspeed;} if(guyvelx[whichguy]>0){guyvelx[whichguy]-=multiplier/slowspeed;}}
		if(activity[whichguy]==jetright||activity[whichguy]==jetleft||activity[whichguy]==jetidle){if(CheckAIKey( whichguy, kDownKey[whichguy] )==1&&CheckAIKey( whichguy, kUpKey[whichguy] )==0){guyvely[whichguy]=(-.4+guyvely[whichguy]/multiplier*100)/(1/multiplier*100+1);}}
		if(activity[whichguy]==jetup&&CheckAIKey( whichguy, kUpKey[whichguy] )==0){activity[whichguy]=jetidle; targetanim[whichguy]=35;}
		if(activity[whichguy]==jetright&&CheckAIKey( whichguy, kRightKey[whichguy] )==0){activity[whichguy]=jetidle; targetanim[whichguy]=35;}
		if(activity[whichguy]==jetleft&&CheckAIKey( whichguy, kLeftKey[whichguy] )==0){activity[whichguy]=jetidle; targetanim[whichguy]=35;}
		if(activity[whichguy]==jetup||activity[whichguy]==jetright||activity[whichguy]==jetleft||activity[whichguy]==jetidle){usingjetpack[whichguy]=1;}
		if(activity[whichguy]==jetup&&activity[whichguy]==jetright&&activity[whichguy]==jetleft&&activity[whichguy]==jetidle){usingjetpack[whichguy]=0;}
		if (activity[whichguy]==deadfaceup&&health[whichguy]>0){activity[whichguy]=getupfromback; targetframe[whichguy]=1; frame[whichguy]=0; targetanim[whichguy]=24; anim[whichguy]=24; time[whichguy]=100;}
		if (activity[whichguy]==deadfacedown&&health[whichguy]>0){activity[whichguy]=getupfromfront; targetframe[whichguy]=1; frame[whichguy]=0; targetanim[whichguy]=25; anim[whichguy]=25; time[whichguy]=100;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==10){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==12){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==10&&itemnum[whichguy]==4&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==12&&itemnum[whichguy]==4&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==1&&anim[whichguy]==10&&itemnum[whichguy]==7&&ammo[whichguy]>0&&lightningdelay[whichguy]<=0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==1&&anim[whichguy]==12&&itemnum[whichguy]==7&&ammo[whichguy]>0&&lightningdelay[whichguy]<=0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==1&&targetframe[whichguy]>3&&anim[whichguy]==10&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]>3&&anim[whichguy]==12&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==10&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==12&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&ammo[whichguy]<=0&&targetframe[whichguy]==0&&anim[whichguy]==10){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootright&&ammo[whichguy]<=0&&targetframe[whichguy]==0&&anim[whichguy]==12){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==31){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==32){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==31&&itemnum[whichguy]==4&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==32&&itemnum[whichguy]==4&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==1&&targetframe[whichguy]>3&&anim[whichguy]==31&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]>3&&anim[whichguy]==32&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==31&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==32&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&ammo[whichguy]<=0&&targetframe[whichguy]==0&&anim[whichguy]==31){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootright&&ammo[whichguy]<=0&&targetframe[whichguy]==0&&anim[whichguy]==32){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==30&&usingjetpack[whichguy]==0){activity[whichguy]=falling;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==30&&usingjetpack[whichguy]==0){activity[whichguy]=falling;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==30&&usingjetpack[whichguy]!=0){activity[whichguy]=jetidle; targetanim[whichguy]=35;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==30&&usingjetpack[whichguy]!=0){activity[whichguy]=jetidle; targetanim[whichguy]=35;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==30&&itemnum[whichguy]==4&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==30&&itemnum[whichguy]==4&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==1&&targetframe[whichguy]>3&&anim[whichguy]==30&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]>3&&anim[whichguy]==30&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==1&&anim[whichguy]==30&&itemnum[whichguy]==7&&ammo[whichguy]>0&&lightningdelay[whichguy]<=0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==1&&anim[whichguy]==30&&itemnum[whichguy]==7&&ammo[whichguy]>0&&lightningdelay[whichguy]<=0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&CheckAIKey( whichguy, kRightShootKey[whichguy] )==1&&targetframe[whichguy]==3&&anim[whichguy]==30&&itemnum[whichguy]==5&&ammo[whichguy]>0&&time[whichguy]>=70){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&CheckAIKey( whichguy, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]==3&&anim[whichguy]==30&&itemnum[whichguy]==5&&ammo[whichguy]>0&&time[whichguy]>=70){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&ammo[whichguy]<=0&&targetframe[whichguy]==0&&anim[whichguy]==30){activity[whichguy]=falling; if(usingjetpack[whichguy]==1){activity[whichguy]=jetidle; targetanim[whichguy]=35;}}
		if (activity[whichguy]==shootright&&ammo[whichguy]<=0&&targetframe[whichguy]==0&&anim[whichguy]==30){activity[whichguy]=falling; if(usingjetpack[whichguy]==1){activity[whichguy]=jetidle; targetanim[whichguy]=35;}}
		if (activity[whichguy]==climbrope&&CheckAIKey( whichguy, kUpKey[whichguy] )==0){activity[whichguy]=onrope;}
		if (activity[whichguy]==downrope&&CheckAIKey( whichguy, kDownKey[whichguy])==0){activity[whichguy]=onrope;}
		if (activity[whichguy]==climbrope){guyy[whichguy]+=multiplier/20*size[whichguy];}
		...
		if (activity[whichguy]==swordthrownleft&&anim[whichguy]==21&&frame[whichguy]>=10/*&&Map[a][guymapy[whichguy]]!=1&&Map[a][guymapy[whichguy]]!=2*/){activity[whichguy]=landonback; time[whichguy]=0; guyx[whichguy]+=12; anim[whichguy]=14; targetanim[whichguy]=14; frame[whichguy]=1; targetframe[whichguy]=1;time[whichguy]=100; rotation[whichguy]*=-1; targetrotation[whichguy]*=-1; guyvelx[whichguy]=.06; guyvely[whichguy]=-.3; health[whichguy]-=120;}
		if (activity[whichguy]==swordthrownright&&anim[whichguy]==21&&frame[whichguy]>=10/*&&Map[b][guymapy[whichguy]]!=1&&Map[b][guymapy[whichguy]]!=2*/){activity[whichguy]=landonback; time[whichguy]=0; guyx[whichguy]-=12; anim[whichguy]=14; targetanim[whichguy]=14; frame[whichguy]=1; targetframe[whichguy]=1;time[whichguy]=100; rotation[whichguy]*=-1; targetrotation[whichguy]*=-1; guyvelx[whichguy]=-.06; guyvely[whichguy]=-.3; health[whichguy]-=120;}
		if (activity[whichguy]==swordthrowleft&&anim[whichguy]==20&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1;}
		
		if (activity[whichguy]==swordthrowright&&anim[whichguy]==20&&targetframe[whichguy]==0&&frame[whichguy]>3){activity[whichguy]=idle1;}
		if (activity[whichguy]==jumpstraight){targetanim[whichguy]=6;}
		if (activity[whichguy]==wallhit&&targetframe[whichguy]==0&&anim[whichguy]==11){activity[whichguy]=idle1;}
		if (activity[whichguy]==jumpstraight&&targetframe[whichguy]==0&&anim[whichguy]==6){activity[whichguy]=falling; guyvely[whichguy]=jumpstrength/5;}
		if (activity[whichguy]==leftflip&&targetframe[whichguy]<4&&guyvely[whichguy]==0){guyvely[whichguy]=jumpstrength/5; guyvelx[whichguy]=-.2;}
		if (activity[whichguy]==leftflip&&targetframe[whichguy]==0&&anim[whichguy]==7&&frame[whichguy]==12){activity[whichguy]=falling; targetframe[whichguy]=2; targetanim[whichguy]=4;}
		if (activity[whichguy]==leftflip&&targetframe[whichguy]==12&&anim[whichguy]==7&&frame[whichguy]==11){frame[whichguy]=12; targetframe[whichguy]=12; time[whichguy]=100;}
		if (activity[whichguy]==rightflip&&targetframe[whichguy]<4&&guyvely[whichguy]==0){guyvely[whichguy]=jumpstrength/5; guyvelx[whichguy]=.2;}
		if (activity[whichguy]==rightflip&&targetframe[whichguy]==0&&anim[whichguy]==7&&frame[whichguy]==12){activity[whichguy]=falling; targetframe[whichguy]=2; targetanim[whichguy]=4;}
		if (activity[whichguy]==rightflip&&targetframe[whichguy]==12&&anim[whichguy]==7&&frame[whichguy]==11){frame[whichguy]=12; targetframe[whichguy]=12; time[whichguy]=100;}
		if (activity[whichguy]==hitwallright&&targetframe[whichguy]==0){time[whichguy]=0; activity[whichguy]=deadfacedown; if(health[whichguy]>0&&verydead==0){activity[whichguy]=getupfromfront; targetframe[whichguy]=1; frame[whichguy]=0; targetanim[whichguy]=25; anim[whichguy]=25; time[whichguy]=100;}}
		if (activity[whichguy]==hitwallleft&&targetframe[whichguy]==0){time[whichguy]=0; activity[whichguy]=deadfacedown; if(health[whichguy]>0&&verydead==0){activity[whichguy]=getupfromfront; targetframe[whichguy]=1; frame[whichguy]=0; targetanim[whichguy]=25; anim[whichguy]=25; time[whichguy]=100;}}
		if (activity[whichguy]==deadfacedown||activity[whichguy]==deadfaceup){if(health[whichguy]>0){time[whichguy]=0; targetframe[whichguy]=2; targetanim[whichguy]=3; activity[whichguy]=ducking;}}
		if (activity[whichguy]==swordidleright||activity[whichguy]==swordidleleft){targetanim[whichguy]=26;}
		if (activity[whichguy]==grenidleright||activity[whichguy]==grenidleleft){targetanim[whichguy]=43;}
		//going down rope stuff
		if (activity[whichguy]==downrope){
			guyy[whichguy]-=multiplier/7*size[whichguy]; if(Map[guymapx[whichguy]][guymapy[whichguy]]==1&&Map[guymapx[whichguy]][guywillbe]==3){activity[whichguy]=falling;guyvely[whichguy]=-.2;targetframe[whichguy]=0;}
			b=-1;
			for (a=0; a<numplayers; a++){
			...
			if(b!=-1){
				activity[b]=falling; targetframe[b]=0;guyvely[b]=-.1; guyy[b]-=.2;
			}
		}
		
		if(guyy[whichguy]>390&&guyvely[whichguy]>=0&&cube==1){guyvely[whichguy]*=-.1; guyy[whichguy]=390;}
		...
		if (activity[whichguy]!=climbrope&&activity[whichguy]!=onrope&&activity[whichguy]!=downrope&&guymapx[whichguy]<100&&guymapx[whichguy]>=0&&guymapy[whichguy]<100&&guymapy[whichguy]>=0){
			if(activity[whichguy]!=jetidle&&activity[whichguy]!=jetleft&&activity[whichguy]!=jetright&&activity[whichguy]!=jetup&&usingjetpack[whichguy]==0&&activity[whichguy]!=jetimpaledright&&activity[whichguy]!=jetimpaledleft){
				if (activity[whichguy]==runleft||activity[whichguy]==leftroll||activity[whichguy]==swordleftspin){if(Map[guymapx[whichguy]][guymapy[whichguy]]==0||Map[guymapx[whichguy]][guymapy[whichguy]]==3){guyvelx[whichguy]=-.1;}}
				if (activity[whichguy]==runright||activity[whichguy]==rightroll||activity[whichguy]==swordrightspin){if(Map[guymapx[whichguy]][guymapy[whichguy]]==0||Map[guymapx[whichguy]][guymapy[whichguy]]==3){guyvelx[whichguy]=.1;}}
				if (Map[guymapx[whichguy]][guywillbe]==0||Map[guymapx[whichguy]][guymapy[whichguy]]==0||Map[guymapx[whichguy]][guymapy[whichguy]]==3||Map[guymapx[whichguy]][guywillbe]==3||guyvely[whichguy]>0||guywillbe!=guymapy[whichguy]||fallin[whichguy]==1||guyy[whichguy]!=(guymapy[whichguy]-39)*-20+.5){
					guyvely[whichguy]-=gravity*multiplier/40; if (activity[whichguy]!=falling&&anim[whichguy]!=7&&anim[whichguy]!=8){if(activity[whichguy]!=deadfaceup&&activity[whichguy]!=landonback&&activity[whichguy]!=deadfacedown&&activity[whichguy]!=jetimpaledright&&activity[whichguy]!=jetimpaledleft&&activity[whichguy]!=fallbackwardsright&&activity[whichguy]!=fallbackwardsleft&&activity[whichguy]!=fallforwardsright&&activity[whichguy]!=fallforwardsleft&&activity[whichguy]!=splat&&activity[whichguy]!=hitwallright&&activity[whichguy]!=hitwallleft&&anim[whichguy]!=30&&targetanim[whichguy]!=30){activity[whichguy]=falling; targetframe[whichguy]=0;} fallin[whichguy]=1;}
					if(activity[whichguy]==landonback&&rotation[whichguy]==-90&&guyvelx[whichguy]==0&&Walls[guymapx[whichguy]][guymapy[whichguy]]==0){guyvelx[whichguy]=.1;}
					if(activity[whichguy]==landonback&&rotation[whichguy]==90&&guyvelx[whichguy]==0&&Walls[guymapx[whichguy]-1][guymapy[whichguy]]==0){guyvelx[whichguy]=-.1;}
				}
			}
			b=-1;
			for (a=0; a<numplayers; a++){
			...
			if (b!=-1&&guyvely[whichguy]<-.3&&dead[b]==0&&attach[whichguy]==-1){
				...
				if(Map[guymapx[b]][guymapy[b]]==1||Map[guymapx[b]][guymapy[b]]==2){
					...
					nosound=0;
					fallin[b]=0;
					if(guyvelx[whichguy]>0){targetrotation[b]=90;}
					if(guyvelx[whichguy]<0){targetrotation[b]=-90;}
					health[b]+=guyvely[b]*70; activity[b]=splat;anim[b]=9; targetframe[b]=0; frame[b]=0; time[b]=100; targetanim[b]=9; playsound(3,-guyvely[b]*50+50); playsound(4,-guyvely[b]*50+50);
					guyvelx[b]=0; 
					guyvely[b]=0;
				}
			}
			...
			if (activity[whichguy]==falling||activity[whichguy]==leftflip||activity[whichguy]==rightflip||activity[whichguy]==rightroll||activity[whichguy]==leftroll||fallin[whichguy]==1||activity[whichguy]==jetup||activity[whichguy]==jetright||activity[whichguy]==jetleft||activity[whichguy]==jetidle||usingjetpack[whichguy]==1){
				if (Map[guymapx[whichguy]][guywillbe]!=0&&Map[guymapx[whichguy]][guywillbe]!=3&&Map[guymapx[whichguy]][guywillbe]!=3&&guyvely[whichguy]<0&&Map[guymapx[whichguy]][guymapy[whichguy]]!=0&&Map[guymapx[whichguy]][guymapy[whichguy]]!=3&&guywillbe==guymapy[whichguy]){
						...
						if(activity[whichguy]!=deadfaceup&&activity[whichguy]!=deadfacedown&&activity[whichguy]!=landonback&&activity[whichguy]!=splat&&activity[whichguy]!=fallbackwardsright&&activity[whichguy]!=fallbackwardsleft&&activity[whichguy]!=fallforwardsright&&activity[whichguy]!=fallforwardsleft&&activity[whichguy]!=hitwallleft&&activity[whichguy]!=hitwallright&&activity[whichguy]!=jetimpaledleft&&activity[whichguy]!=jetimpaledright){activity[whichguy]=ducking;}
						...
						if (guyvely[whichguy]<-.7&&c!=jetimpaleleft&&c!=jetimpaleright&&guyvelx[whichguy]<0&&dead[whichguy]==0){activity[whichguy]=leftroll;targetframe[whichguy]=2;}
						if (guyvely[whichguy]<-.7&&c!=jetimpaleleft&&c!=jetimpaleright&&guyvelx[whichguy]>0&&dead[whichguy]==0){activity[whichguy]=rightroll;targetframe[whichguy]=2;}
						if (guyvely[whichguy]<-1){health[whichguy]+=guyvely[whichguy]*70; activity[whichguy]=splat;anim[whichguy]=9; targetframe[whichguy]=0; frame[whichguy]=0; time[whichguy]=100; targetanim[whichguy]=9; playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);}
						if (activity[whichguy]==deadfacedown||activity[whichguy]==fallforwardsright||activity[whichguy]==fallforwardsleft||activity[whichguy]==hitwallright||activity[whichguy]==hitwallleft){ health[whichguy]-=10; if(activity[whichguy]!=fallforwardsright&&activity[whichguy]!=fallforwardsleft){activity[whichguy]=splat;anim[whichguy]=9; targetframe[whichguy]=2; frame[whichguy]=0; time[whichguy]=100; targetanim[whichguy]=9;} playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);}
						if (activity[whichguy]==deadfaceup||activity[whichguy]==fallbackwardsright||activity[whichguy]==fallbackwardsleft||activity[whichguy]==landonback){activity[whichguy]=landonback; if(a==37){targetrotation[whichguy]*=-1; rotation[whichguy]*=-1;} anim[whichguy]=14; targetframe[whichguy]=1; frame[whichguy]=0; time[whichguy]=100; targetanim[whichguy]=14; playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);}
						if (activity[whichguy]==ducking&&nosound==0&&invisible[whichguy]==0){playsound(2,-guyvely[whichguy]*100+100);}
						if (activity[whichguy]==ducking&&nosound==0&&invisible[whichguy]==1){playsound(2,-guyvely[whichguy]*10+10);}
						guyvelx[whichguy]=0; 
						guyvely[whichguy]=0;
						}
						if(c==jetimpaleright||c==jetimpaleleft||activity[whichguy]==jetimpaleright||activity[whichguy]==jetimpaleleft){guyvely[whichguy]=.005;if(c==jetimpaleleft||c==jetimpaleright){activity[whichguy]=c;}}
		...
		if(dead[whichguy]==0/*&&activity[whichguy]!=fallbackwardsright&&activity[whichguy]!=fallbackwardsleft&&activity[whichguy]!=landonback&&activity[whichguy]!=deadfaceup&&activity[whichguy]!=deadfacedown*/){
			...
			if(g==1){g=0;}
			for(e=0;e<=g;e++){
				if (a>c&&Walls[c+e+1][b]==1&&f==0){usingjetpack[whichguy]=0; guyx[whichguy]-=.3+.6*multiplier/5; playsound(2,30); if(guyvelx[whichguy]==0){targetanim[whichguy]=11; activity[whichguy]=wallhit;targetframe[whichguy]=0; time[whichguy]=100;} if(guyvelx[whichguy]!=0){activity[whichguy]=leftroll; anim[whichguy]=8; targetanim[whichguy]=8; guyvelx[whichguy]=guyvelx[whichguy]*-.5; targetframe[whichguy]=1; frame[whichguy]=1; time[whichguy]=100; }f=1;}
				if (a>c&&Walls[c+e+1][d]==1&&f==0){usingjetpack[whichguy]=0; guyx[whichguy]-=.3+.6*multiplier/5; playsound(2,30); if(guyvelx[whichguy]==0){targetanim[whichguy]=11; activity[whichguy]=wallhit;targetframe[whichguy]=0; time[whichguy]=100;} if(guyvelx[whichguy]!=0){activity[whichguy]=leftroll; anim[whichguy]=8; targetanim[whichguy]=8; guyvelx[whichguy]=guyvelx[whichguy]*-.5; targetframe[whichguy]=1; frame[whichguy]=1; time[whichguy]=100;}f=1;}
			}
			...
			if(g==1){g=0;}
			for(e=0;e<=g;e++){
				if (a<c&&Walls[c-e-1][b]==1&&f==0){usingjetpack[whichguy]=0; guyx[whichguy]+=.3+.6*multiplier/5; playsound(2,30); if(guyvelx[whichguy]==0){targetanim[whichguy]=11; activity[whichguy]=wallhit;targetframe[whichguy]=0; time[whichguy]=100;} if(guyvelx[whichguy]!=0){activity[whichguy]=rightroll; anim[whichguy]=8; targetanim[whichguy]=8; guyvelx[whichguy]=guyvelx[whichguy]*-.5; targetframe[whichguy]=1; frame[whichguy]=1; time[whichguy]=100;}f=1;}
				if (a<c&&Walls[c-e-1][d]==1&&f==0){usingjetpack[whichguy]=0; guyx[whichguy]+=.3+.6*multiplier/5; playsound(2,30); if(guyvelx[whichguy]==0){targetanim[whichguy]=11; activity[whichguy]=wallhit;targetframe[whichguy]=0; time[whichguy]=100;} if(guyvelx[whichguy]!=0){activity[whichguy]=rightroll; anim[whichguy]=8; targetanim[whichguy]=8; guyvelx[whichguy]=guyvelx[whichguy]*-.5; targetframe[whichguy]=1; frame[whichguy]=1; time[whichguy]=100;}f=1;}
			}
		}
		if(dead[whichguy]==1){
			a=(guyx[whichguy]+595)/10-.55;
			...
			if(g==1){g=0;}
			for(e=0;e<=g;e++){
				if (a>c&&Walls[c+e+1][b]==1&&f==0){usingjetpack[whichguy]=0; guyx[whichguy]-=.3+.6*multiplier/5; playsound(2,30); if(guyvelx[whichguy]!=0){activity[whichguy]=fallbackwardsright; guyx[whichguy]=guymapx[whichguy]*10-590; anim[whichguy]=13; targetanim[whichguy]=13;} time[whichguy]=100; guyvelx[whichguy]=guyvelx[whichguy]*-.5; f=1;}
				if (a>c&&Walls[c+e+1][d]==1&&f==0){usingjetpack[whichguy]=0; guyx[whichguy]-=.3+.6*multiplier/5; playsound(2,30); if(guyvelx[whichguy]!=0){activity[whichguy]=fallbackwardsright; guyx[whichguy]=guymapx[whichguy]*10-590; anim[whichguy]=13; targetanim[whichguy]=13;} time[whichguy]=100; guyvelx[whichguy]=guyvelx[whichguy]*-.5; f=1;}
			}
			a=(guyx[whichguy]+595)/10-.3;
			b=guyy[whichguy]/-20-.5+40;
			c=(oldguyx[whichguy]+595)/10-.3;
			...
			if(g==1){g=0;}
			for(e=0;e<=g;e++){
				if (a<c&&Walls[a][b]==1&&f==0){usingjetpack[whichguy]=0; guyx[whichguy]+=.3+.6*multiplier/5; playsound(2,30); if(guyvelx[whichguy]!=0){activity[whichguy]=fallbackwardsleft; guyx[whichguy]=guymapx[whichguy]*10-590; anim[whichguy]=13; targetanim[whichguy]=13;} time[whichguy]=100; guyvelx[whichguy]=guyvelx[whichguy]*-.5; f=1;}
				if (a<c&&Walls[a][d]==1&&f==0){usingjetpack[whichguy]=0; guyx[whichguy]+=.3+.6*multiplier/5; playsound(2,30); if(guyvelx[whichguy]!=0){activity[whichguy]=fallbackwardsleft; guyx[whichguy]=guymapx[whichguy]*10-590; anim[whichguy]=13; targetanim[whichguy]=13;} time[whichguy]=100; guyvelx[whichguy]=guyvelx[whichguy]*-.5; f=1;}
			}
		}
		if (health[whichguy]<=0){dead[whichguy]=1;}
		...
		if(health[whichguy]<=0&&oldhealth[whichguy]>0){deathcount[oldnum[whichguy]]++; if(autorespawn==1){restartRound();}}
		if(health[whichguy]<oldhealth[whichguy]){usingjetpack[whichguy]=0;}
		...
		if(jetpack[whichguy]==1&&usingjetpack[whichguy]==1){
			...
			if(usingjetpack[whichguy]==1){sprite[jetglownum[whichguy]].brightness=RangedRandom(200,255);}
			if(usingjetpack[whichguy]==1&&activity[whichguy]==jetidle){sprite[jetglownum[whichguy]].brightness=RangedRandom(100,150);}
			jetsmokedelay[whichguy]-=multiplier;
		}
		if(jetpack[whichguy]==0||usingjetpack[whichguy]==0){
			if(jetglownum[whichguy]!=0){sprite[jetglownum[whichguy]].type=0; jetglownum[whichguy]=0;}
		}
	//if(lightningfxdelay[whichguy]<=0){lightningfxdelay[whichguy]=7;}
	for(x=0;x<numplayers;x++){
		if(activity[x]!=jetimpaledright&&activity[x]!=jetimpaledleft&&attach[x]==whichguy){attach[x]=-1;}
	}
}
...
void DoPlayerStuff(int whichguy){
	...
	if(health[whichguy]<=0&&(activity[whichguy]==idle1||activity[whichguy]==idle2||activity[whichguy]==jetidle||activity[whichguy]==grenidleright||activity[whichguy]==grenidleleft||activity[whichguy]==swordidleleft||activity[whichguy]==swordidleright)){
		activity[whichguy]=splat;anim[whichguy]=9; targetframe[whichguy]=0; frame[whichguy]=0; time[whichguy]=100; targetanim[whichguy]=9; playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);
	}
	if(activity[whichguy]==impaleright||activity[whichguy]==impaleleft||
	activity[whichguy]==swordrightthrust||activity[whichguy]==swordleftthrust||
	activity[whichguy]==swordrightspin||activity[whichguy]==swordleftspin){
	...
	int guywillbe;
		...
		if (activity[whichguy]!=climbrope&&activity[whichguy]!=onrope&&activity[whichguy]!=downrope){
			if (activity[whichguy]==idle1){targetanim[whichguy]=0;}
			if (activity[whichguy]==idle2){targetanim[whichguy]=1;}
			if (activity[whichguy]==runleft&&IsKeyDown( theKeyMap, kLeftKey[whichguy] )==0){activity[whichguy]=RangedRandom(1,2); time[whichguy]=0;}
			if (activity[whichguy]==runright&&IsKeyDown( theKeyMap, kRightKey[whichguy] )==0){activity[whichguy]=RangedRandom(1,2); time[whichguy]=0;}
			if (activity[whichguy]==ducking&&IsKeyDown( theKeyMap, kDownKey[whichguy] )==0){activity[whichguy]=RangedRandom(1,2); time[whichguy]=0;}
			if (activity[whichguy]==runleft){targetanim[whichguy]=2; guyx[whichguy]-=.6*multiplier/5*size[whichguy];}
			if (activity[whichguy]==runright){targetanim[whichguy]=2; guyx[whichguy]+=.6*multiplier/5*size[whichguy];}
			if (activity[whichguy]==swordleftspin&&frame[whichguy]<=2){guyx[whichguy]-=.3*multiplier/5*size[whichguy];}
			if (activity[whichguy]==swordrightspin&&frame[whichguy]<=2){guyx[whichguy]+=.3*multiplier/5*size[whichguy];}
		...
		if(activity[whichguy]==fallforwardsleft&&targetframe[whichguy]<=5&&guyvelx[whichguy]==0){guyx[whichguy]-=.6*multiplier/5*size[whichguy];}
		if(activity[whichguy]==fallforwardsright&&targetframe[whichguy]<=5&&guyvelx[whichguy]==0){guyx[whichguy]+=.6*multiplier/5*size[whichguy];}
		if(activity[whichguy]==jetright){guyvelx[whichguy]=(.4+guyvelx[whichguy]/multiplier*100)/(1/multiplier*100+1); if(IsKeyDown( theKeyMap, kUpKey[whichguy] )==0&&IsKeyDown( theKeyMap, kDownKey[whichguy] )==0){if(absolute(guyvely[whichguy])<multiplier/slowspeed){guyvely[whichguy]=0;}if(guyvely[whichguy]<0){guyvely[whichguy]+=multiplier/slowspeed;} if(guyvely[whichguy]>0){guyvely[whichguy]-=multiplier/slowspeed;}}}
		if(activity[whichguy]==jetleft){guyvelx[whichguy]=(-.4+guyvelx[whichguy]/multiplier*100)/(1/multiplier*100+1); if(IsKeyDown( theKeyMap, kUpKey[whichguy] )==0&&IsKeyDown( theKeyMap, kDownKey[whichguy] )==0){if(absolute(guyvely[whichguy])<multiplier/slowspeed){guyvely[whichguy]=0;}if(guyvely[whichguy]<0){guyvely[whichguy]+=multiplier/slowspeed;} if(guyvely[whichguy]>0){guyvely[whichguy]-=multiplier/slowspeed;}}}
		if(activity[whichguy]==jetup||activity[whichguy]==jetright||activity[whichguy]==jetleft||activity[whichguy]==jetidle){if(IsKeyDown( theKeyMap, kUpKey[whichguy] )==1){guyvely[whichguy]=(.4+guyvely[whichguy]/multiplier*100)/(1/multiplier*100+1);}}
		if(activity[whichguy]==jetup){if(absolute(guyvely[whichguy])<multiplier/slowspeed){guyvelx[whichguy]=0;}if(guyvelx[whichguy]<0){guyvelx[whichguy]+=multiplier/slowspeed;} if(guyvelx[whichguy]>0){guyvelx[whichguy]-=multiplier/slowspeed;}}
		if(activity[whichguy]==jetright||activity[whichguy]==jetleft||activity[whichguy]==jetidle){if(IsKeyDown( theKeyMap, kDownKey[whichguy] )==1&&IsKeyDown( theKeyMap, kUpKey[whichguy] )==0){guyvely[whichguy]=(-.4+guyvely[whichguy]/multiplier*100)/(1/multiplier*100+1);}}
		if(activity[whichguy]==jetup&&IsKeyDown( theKeyMap, kUpKey[whichguy] )==0){activity[whichguy]=jetidle; targetanim[whichguy]=35;}
		if(activity[whichguy]==jetright&&IsKeyDown( theKeyMap, kRightKey[whichguy] )==0){activity[whichguy]=jetidle; targetanim[whichguy]=35;}
		if(activity[whichguy]==jetleft&&IsKeyDown( theKeyMap, kLeftKey[whichguy] )==0){activity[whichguy]=jetidle; targetanim[whichguy]=35;}
		if(activity[whichguy]==jetup||activity[whichguy]==jetright||activity[whichguy]==jetleft||activity[whichguy]==jetidle){usingjetpack[whichguy]=1;}
		if(activity[whichguy]==jetup&&activity[whichguy]==jetright&&activity[whichguy]==jetleft&&activity[whichguy]==jetidle){usingjetpack[whichguy]=0;}
		if (activity[whichguy]==deadfaceup&&health[whichguy]>0){activity[whichguy]=getupfromback; targetframe[whichguy]=1; frame[whichguy]=0; targetanim[whichguy]=24; anim[whichguy]=24; time[whichguy]=100;}
		if (activity[whichguy]==deadfacedown&&health[whichguy]>0){activity[whichguy]=getupfromfront; targetframe[whichguy]=1; frame[whichguy]=0; targetanim[whichguy]=25; anim[whichguy]=25; time[whichguy]=100;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==10){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==12){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==10&&itemnum[whichguy]==4&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==12&&itemnum[whichguy]==4&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==1&&anim[whichguy]==10&&itemnum[whichguy]==7&&ammo[whichguy]>0&&lightningdelay[whichguy]<=0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==1&&anim[whichguy]==12&&itemnum[whichguy]==7&&ammo[whichguy]>0&&lightningdelay[whichguy]<=0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==1&&targetframe[whichguy]>3&&anim[whichguy]==10&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]>3&&anim[whichguy]==12&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==10&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==12&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&ammo[whichguy]<=0&&targetframe[whichguy]==0&&anim[whichguy]==10){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootright&&ammo[whichguy]<=0&&targetframe[whichguy]==0&&anim[whichguy]==12){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==31){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==32){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==31&&itemnum[whichguy]==4&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==32&&itemnum[whichguy]==4&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==1&&targetframe[whichguy]>3&&anim[whichguy]==31&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]>3&&anim[whichguy]==32&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==31&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==32&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&ammo[whichguy]<=0&&targetframe[whichguy]==0&&anim[whichguy]==31){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootright&&ammo[whichguy]<=0&&targetframe[whichguy]==0&&anim[whichguy]==32){activity[whichguy]=idle1;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==30&&usingjetpack[whichguy]==0){activity[whichguy]=falling;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==30&&usingjetpack[whichguy]==0){activity[whichguy]=falling;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==30&&usingjetpack[whichguy]!=0){activity[whichguy]=jetidle; targetanim[whichguy]=35;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==0&&targetframe[whichguy]==0&&anim[whichguy]==30&&usingjetpack[whichguy]!=0){activity[whichguy]=jetidle; targetanim[whichguy]=35;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==30&&itemnum[whichguy]==4&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]==0&&anim[whichguy]==30&&itemnum[whichguy]==4&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==1&&targetframe[whichguy]>3&&anim[whichguy]==30&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]>3&&anim[whichguy]==30&&itemnum[whichguy]==5&&ammo[whichguy]>0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==1&&anim[whichguy]==30&&itemnum[whichguy]==7&&ammo[whichguy]>0&&lightningdelay[whichguy]<=0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==1&&anim[whichguy]==30&&itemnum[whichguy]==7&&ammo[whichguy]>0&&lightningdelay[whichguy]<=0){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&IsKeyDown( theKeyMap, kRightShootKey[whichguy] )==1&&targetframe[whichguy]==3&&anim[whichguy]==30&&itemnum[whichguy]==5&&ammo[whichguy]>0&&time[whichguy]>=70){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootleft;}
		if (activity[whichguy]==shootright&&IsKeyDown( theKeyMap, kLeftShootKey[whichguy] )==1&&targetframe[whichguy]==3&&anim[whichguy]==30&&itemnum[whichguy]==5&&ammo[whichguy]>0&&time[whichguy]>=70){targetframe[whichguy]=2; frame[whichguy]=3; time[whichguy]=100; activity[whichguy]=shootright;}
		if (activity[whichguy]==shootleft&&ammo[whichguy]<=0&&targetframe[whichguy]==0&&anim[whichguy]==30){activity[whichguy]=falling; if(usingjetpack[whichguy]==1){activity[whichguy]=jetidle; targetanim[whichguy]=35;}}
		if (activity[whichguy]==shootright&&ammo[whichguy]<=0&&targetframe[whichguy]==0&&anim[whichguy]==30){activity[whichguy]=falling; if(usingjetpack[whichguy]==1){activity[whichguy]=jetidle; targetanim[whichguy]=35;}}
		if (activity[whichguy]==climbrope&&IsKeyDown( theKeyMap, kUpKey[whichguy] )==0){activity[whichguy]=onrope;}
		if (activity[whichguy]==downrope&&IsKeyDown( theKeyMap, kDownKey[whichguy])==0){activity[whichguy]=onrope;}
		if (activity[whichguy]==climbrope){guyy[whichguy]+=multiplier/20*size[whichguy];}
		//if (activity[whichguy]==painoneleft&&anim[whichguy]==13&&targetframe[whichguy]==0){activity[whichguy]=landonback; guyx[whichguy]-=7*size[whichguy]; targetanim[whichguy]=14; anim[whichguy]=14; targetframe[whichguy]=0; frame[whichguy]=0; time[whichguy]=100; if(Map[guymapx[whichguy]][guymapy[whichguy]]!=0&&Map[guymapx[whichguy]][guymapy[whichguy]]!=3&&activity[whichguy]!=climbrope&&activity[whichguy]!=onrope&&activity[whichguy]!=downrope){playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);}}
		/*if (activity[whichguy]==swordthrownleft&&anim[whichguy]==21&&frame[whichguy]>=10&&Map[guymapx[whichguy]+2][guymapy[whichguy]]==0){activity[whichguy]=landonback; dead[whichguy]=1; time[whichguy]=0; guyx[whichguy]+=12; anim[whichguy]=14; targetanim[whichguy]=14; frame[whichguy]=1; targetframe[whichguy]=1;time[whichguy]=100; rotation[whichguy]*=-1; targetrotation[whichguy]*=-1; guyvelx[whichguy]=.06; guyvely[whichguy]=-.3;}
		if (activity[whichguy]==swordthrownleft&&anim[whichguy]==21&&frame[whichguy]>=10&&Map[guymapx[whichguy]+2][guymapy[whichguy]]!=0&&Map[guymapx[whichguy]+1][guymapy[whichguy]]==0){activity[whichguy]=landonback; dead[whichguy]=1; time[whichguy]=0; guyx[whichguy]+=12; anim[whichguy]=14; targetanim[whichguy]=14; frame[whichguy]=1; targetframe[whichguy]=1;time[whichguy]=100; rotation[whichguy]*=-1; targetrotation[whichguy]*=-1; guyvelx[whichguy]=.06; guyvely[whichguy]=-.3;}
		...
		if (activity[whichguy]!=climbrope&&activity[whichguy]!=onrope&&activity[whichguy]!=downrope&&guymapx[whichguy]<100&&guymapx[whichguy]>=0&&guymapy[whichguy]<100&&guymapy[whichguy]>=0){
			...
			if (activity[whichguy]==falling||activity[whichguy]==leftflip||activity[whichguy]==rightflip||activity[whichguy]==rightroll||activity[whichguy]==leftroll||fallin[whichguy]==1||activity[whichguy]==jetup||activity[whichguy]==jetright||activity[whichguy]==jetleft||activity[whichguy]==jetidle||usingjetpack[whichguy]==1){
				if (Map[guymapx[whichguy]][guywillbe]!=0&&Map[guymapx[whichguy]][guywillbe]!=3&&Map[guymapx[whichguy]][guywillbe]!=3&&guyvely[whichguy]<0&&Map[guymapx[whichguy]][guymapy[whichguy]]!=0&&Map[guymapx[whichguy]][guymapy[whichguy]]!=3&&guywillbe==guymapy[whichguy]&&attach[whichguy]==-1){
						...
						if(activity[whichguy]!=deadfaceup&&activity[whichguy]!=deadfacedown&&activity[whichguy]!=landonback&&activity[whichguy]!=splat&&activity[whichguy]!=fallbackwardsright&&activity[whichguy]!=fallbackwardsleft&&activity[whichguy]!=fallforwardsright&&activity[whichguy]!=fallforwardsleft&&activity[whichguy]!=hitwallleft&&activity[whichguy]!=hitwallright&&activity[whichguy]!=jetimpaledleft&&activity[whichguy]!=jetimpaledright){activity[whichguy]=ducking;}
						if(IsKeyDown( theKeyMap, kLeftKey[whichguy] )&&dead[whichguy]==0){activity[whichguy]=leftroll;targetframe[whichguy]=2;} 
						if(IsKeyDown( theKeyMap, kRightKey[whichguy] )&&dead[whichguy]==0){activity[whichguy]=rightroll;targetframe[whichguy]=2;} 
						if (guyvely[whichguy]<-.7&&c!=jetimpaleleft&&c!=jetimpaleright&&guyvelx[whichguy]<0&&dead[whichguy]==0){activity[whichguy]=leftroll;targetframe[whichguy]=2;}
						if (guyvely[whichguy]<-.7&&c!=jetimpaleleft&&c!=jetimpaleright&&guyvelx[whichguy]>0&&dead[whichguy]==0){activity[whichguy]=rightroll;targetframe[whichguy]=2;}
						if (guyvely[whichguy]<-1){health[whichguy]+=guyvely[whichguy]*70; activity[whichguy]=splat;anim[whichguy]=9; targetframe[whichguy]=0; frame[whichguy]=0; time[whichguy]=100; targetanim[whichguy]=9; playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);}
						if (activity[whichguy]==deadfacedown||activity[whichguy]==fallforwardsright||activity[whichguy]==fallforwardsleft||activity[whichguy]==hitwallright||activity[whichguy]==hitwallleft){ health[whichguy]-=10; if(activity[whichguy]!=fallforwardsright&&activity[whichguy]!=fallforwardsleft){activity[whichguy]=splat;anim[whichguy]=9; targetframe[whichguy]=2; frame[whichguy]=0; time[whichguy]=100; targetanim[whichguy]=9;} playsound(3,-guyvely[whichguy]*50+50); playsound(4,-guyvely[whichguy]*50+50);}
			...
			slide+=multiplier/100;
			if (slide>maxsprites){slide=1000;}
			guyx[whichguy]+=guyvelx[whichguy]*multiplier;
		if(dead[whichguy]==0/*&&activity[whichguy]!=fallbackwardsright&&activity[whichguy]!=fallbackwardsleft&&activity[whichguy]!=landonback&&activity[whichguy]!=deadfaceup&&activity[whichguy]!=deadfacedown*/){
			a=(guyx[whichguy]+595)/10-.55;
			b=guyy[whichguy]/-20-.5+40;
		...
		if (health[whichguy]<=0){dead[whichguy]=1;}
		//if (guyx<-395){guyx+=multiplier/3; activity=rightroll; anim=8; targetanim=8; if(guyvelx[whichguy]!=0){guyvelx=.1;} targetframe=1; frame=1; time=100;}
		if(health[whichguy]<=0&&oldhealth[whichguy]>0){deathcount[oldnum[whichguy]]++;}
		if(health[whichguy]<oldhealth[whichguy]){usingjetpack[whichguy]=0;}
		if(guymapy[whichguy]>=99||guymapx[whichguy]>=99||guymapx[whichguy]<=1||guyy[whichguy]>390){
			if(exists[whichguy]&&cube==0){
			for(x=0;x<50;x++){
	...
	//if(lightningfxdelay[whichguy]<=0){lightningfxdelay[whichguy]=7;}
	if(activity[whichguy]!=jetimpaledright&&activity[whichguy]!=jetimpaledleft){attach[whichguy]=-1;}
}
...
void ShootGrenade(int whichguy)
{
	...
	if(soundvolume>200){soundvolume=200;}
	...
	if(facing[whichguy]==1&&Walls[g][c]==0){e=1;}
	if(facing[whichguy]==0&&Walls[h-2][c]==0){e=1;}
	...
	if(b!=-1){d=(guyx[b]+595)/10;}
	if(b!=-1&&facing[whichguy]==1){
		if(a==d||Walls[a][c]==0){
		fake[whichguy]=0;
		guyx[b]=guyx[whichguy]; attacker[b]=whichguy;
		guyy[b]=guyy[whichguy];
		usingjetpack[b]=0;
		guyvelx[b]=0;
		guyvely[b]=0; fallin[b]=0;
		activity[whichguy]=breakneckright; anim[whichguy]=16; targetanim[whichguy]=16;
		activity[b]=brokeneckright; anim[b]=17; targetanim[b]=17;  usingjetpack[b]=0;
		targetframe[whichguy]=1; targetframe[b]=1;
		frame[whichguy]=0; frame[b]=0;
		time[whichguy]=0; time[b]=0;
		}
	}
	...
	if(b!=-1&&facing[whichguy]==0){
		...
		guyvelx[b]=0;
		guyvely[b]=0; fallin[b]=0;
		activity[whichguy]=breakneckleft; anim[whichguy]=16; targetanim[whichguy]=16;
		activity[b]=brokeneckleft; anim[b]=17; targetanim[b]=17;
		targetframe[whichguy]=1; targetframe[b]=1;
		frame[whichguy]=0; frame[b]=0;
		time[whichguy]=0; time[b]=0;
		 usingjetpack[b]=0;
	...
	if(itemnum[whichguy]==6&&ammo[whichguy]>0&&usingjetpack[whichguy]==0&&e==1&&b==-1){
		...
		time[whichguy]=0;
		if(smokingbody[whichguy][kRightGun]<100){smokingbody[whichguy][kRightGun]+=50;}
		if(facing[whichguy]==1){
			targetrotation[whichguy]=90; rotation[whichguy]=90;
		...
		point=FindRightGunPoint(whichguy); 
		if(holding[whichguy]>100){holding[whichguy]=100;}
		MakeSprite(point.x,point.y,point.z,200,grenade, 1, 10, 0,-holding[whichguy]+(facing[whichguy]*holding[whichguy]*2),.1,0);
		if(itemtype[whichguy]==0){ammo[whichguy]-=5;}
		if(itemtype[whichguy]==1){ammo[whichguy]-=5; sprite[unusedsprite].type2=1; /*ammo[whichguy]-=0;*/ sprite[unusedsprite].owner=whichguy;}
	}
	}
	if(itemnum[whichguy]==6&&anim[whichguy]!=30&&ammo[whichguy]>0&&usingjetpack[whichguy]==1&&(activity[whichguy]==jetidle||activity[whichguy]==jetleft||activity[whichguy]==jetright||activity[whichguy]==jetup||activity[whichguy]==jetdown)){
		...
		if(facing[whichguy]==1){
			...
			activity[whichguy]=shootright; targetframe[whichguy]=3; frame[whichguy]=2; targetanim[whichguy]=30; anim[whichguy]=30; time[whichguy]=0; targetrotation[whichguy]=0;
			targetrotation[whichguy]=90; rotation[whichguy]=90;
			point=FindRightGunPoint(whichguy); 
			if (freezetime==0){
			for (x=0;x<16;x++){
		...
		if(facing[whichguy]==0){
			guyvelx[whichguy]+=holding[whichguy]/200;
			activity[whichguy]=shootleft; targetframe[whichguy]=3; frame[whichguy]=2; targetanim[whichguy]=30; anim[whichguy]=30; time[whichguy]=0; targetrotation[whichguy]=0;
			targetrotation[whichguy]=-90; rotation[whichguy]=-90;
			point=FindRightGunPoint(whichguy); 
			if (freezetime==0){
			for (x=0;x<16;x++){
		...
		MakeSprite(point.x,point.y,point.z,200,grenade, 1, 10, 0,-holding[whichguy]+(facing[whichguy]*holding[whichguy]*2),.1,0);
		if(itemtype[whichguy]==0){ammo[whichguy]-=5;}
		if(itemtype[whichguy]==1){ammo[whichguy]-=5; 
		sprite[unusedsprite].type2=1; /*ammo[whichguy]-=0;*/ sprite[unusedsprite].owner=whichguy;}
	}
}
...
int CheckPaths(int whichguy, int num){
	...
	if(compxsteptest[whichguy][num]==computertargetxlocation[whichguy]&&compysteptest[whichguy][num]==computertargetylocation[whichguy]&&num<pathfindsteps){
		pathfound[whichguy]=1; pathfindsteps=num;
		for(a=0;a<=100;a++){
	...
	if((Map[compxsteptest[whichguy][num]+1][compysteptest[whichguy][num]]==1||Map[compxsteptest[whichguy][num]+1][compysteptest[whichguy][num]]==2||Map[compxsteptest[whichguy][num]+1][compysteptest[whichguy][num]]==3)&&Walls[compxsteptest[whichguy][num]][compysteptest[whichguy][num]]==0){
		...
		if(num<90&&num<pathfindsteps&&haschecked==0&&compxsteptest[whichguy][num+1]!=compxsteptest[whichguy][num-1]){CheckPaths(whichguy,num+1);}
	}
	...
	if(Map[compxsteptest[whichguy][num]][compysteptest[whichguy][num]+1]==3||Map[compxsteptest[whichguy][num]][compysteptest[whichguy][num]+1]==2){
		...
		if(num<90&&num<pathfindsteps&&haschecked==0&&compysteptest[whichguy][num+1]!=compysteptest[whichguy][num-1]){CheckPaths(whichguy,num+1);}
	}
...
void DoAI(int whichguy)
{
	...
	if(!computertarget[whichguy]){
		for(a=0;a<numplayers;a++){
			...
			if((olddistance=1234567||distance<olddistance)&&a!=whichguy&&health[a]>0){olddistance=distance; computertarget[whichguy]=a;}
		}
	}
...
void DoAIKeys(int whichguy)
{
	...
	if ( CheckAIKey(whichguy, kBlockKey[whichguy] ) && delaytime[whichguy]<=0){
		if(activity[whichguy]==idle1||activity[whichguy]==idle2||activity[whichguy]==swordidleright||activity[whichguy]==swordidleleft||activity[whichguy]==grenidleright||activity[whichguy]==grenidleleft||activity[whichguy]==painoneleft||activity[whichguy]==paintwoleft||activity[whichguy]==painoneright||activity[whichguy]==paintworight){
			if(itemnum[whichguy]==2||itemnum[whichguy]==3){
				...
				if(itemnum[whichguy]==3){delaytime[whichguy]=240;}
			}
		}
		if(itemnum[whichguy]==6&&itemtype[whichguy]==1&&health[whichguy]>0){
			for(x=0;x<maxsprites;x++){
				if (sprite[x].owner==whichguy&&sprite[x].type==grenade&&sprite[x].type2==1){Explode(x);}
			}
		}
	}
	//*************ATTACK KEY**************//
	if ( CheckAIKey(whichguy, kAttackKey[whichguy] ) ){
		if(holding[whichguy]==0){holding[whichguy]=1;}
				...
				if(facing[whichguy]==1){
					...
					if(b!=-1&&rightshootpressed[whichguy]==0&&attackpressed[whichguy]==0){
						if(itemnum[whichguy]==2||itemnum[whichguy]==3){
							...
							guyvely[b]=guyvely[whichguy];
							activity[whichguy]=jetimpaleright; anim[whichguy]=36; targetanim[whichguy]=36;
							activity[b]=jetimpaledright; anim[b]=37; targetanim[b]=37;
							targetframe[whichguy]=1; targetframe[b]=1;
							frame[whichguy]=0; frame[b]=0;
							time[whichguy]=0; time[b]=0;
							}
				...
				if(facing[whichguy]==0){
					...
					if(b!=-1&&leftshootpressed[whichguy]==0&&attackpressed[whichguy]==0){
						if(itemnum[whichguy]==2||itemnum[whichguy]==3){
							...
							guyvelx[b]=guyvelx[whichguy];
							guyvely[b]=guyvely[whichguy];
							activity[whichguy]=jetimpaleleft; anim[whichguy]=36; targetanim[whichguy]=36;
							activity[b]=jetimpaledleft; anim[b]=37; targetanim[b]=37;
							targetframe[whichguy]=1; targetframe[b]=1;
							frame[whichguy]=0; frame[b]=0;
							time[whichguy]=0; time[b]=0;
							}
	...
	if ( CheckAIKey(whichguy, kAttackKey[whichguy] ) ==0&&CheckAIKey(whichguy, kRightShootKey[whichguy])==0&&CheckAIKey(whichguy, kLeftShootKey[whichguy] )==0){
		if(holding[whichguy]!=0){
			if(holdingtype[whichguy]==-1){facing[whichguy]=0;}
			if(holdingtype[whichguy]==1){facing[whichguy]=1;}
	...
	if ( CheckAIKey(whichguy, kRightShootKey[whichguy] ) ){
		if(usingjetpack[whichguy]==0){facing[whichguy]=1;}
		if(activity[whichguy]==jetleft||activity[whichguy]==jetright||activity[whichguy]==jetup){if(itemnum[whichguy]>=1&&itemnum[whichguy]<=5){activity[whichguy]=jetidle;}}
		if(activity[whichguy]==idle1||activity[whichguy]==idle2||activity[whichguy]==swordidleright||activity[whichguy]==swordidleleft||activity[whichguy]==grenidleright||activity[whichguy]==grenidleleft||activity[whichguy]==jetidle||activity[whichguy]==ducking||activity[whichguy]==falling){
			...
			if(b==-1&&ammo[whichguy]>0&&itemnum[whichguy]==1&&Walls[g][c]==0){
				...
				if(Walls[a][c]==0&&activity[whichguy]!=falling&&activity[whichguy]!=jetidle&&activity[whichguy]!=ducking){
					activity[whichguy]=shootleft; targetframe[whichguy]=0; targetanim[whichguy]=10; time[whichguy]=70; targetrotation[whichguy]=0;
				}
				if(Walls[a][c]==0&&activity[whichguy]==falling){
					activity[whichguy]=shootleft; targetframe[whichguy]=0; targetanim[whichguy]=30; time[whichguy]=100; targetrotation[whichguy]=0;
			...
			if(b==-1&&ammo[whichguy]>0&&itemnum[whichguy]==5&&Walls[g][c]==0){
				...
				c=guyy[whichguy]/-20-.5+40;
				if(Walls[a][c]==0&&activity[whichguy]!=falling&&activity[whichguy]!=jetidle&&activity[whichguy]!=ducking){
					activity[whichguy]=shootleft; targetframe[whichguy]=0; targetanim[whichguy]=31; time[whichguy]=70; targetrotation[whichguy]=0;
				}
				if(Walls[a][c]==0&&activity[whichguy]==falling){
					activity[whichguy]=shootleft; targetframe[whichguy]=0; targetanim[whichguy]=30; time[whichguy]=100; targetrotation[whichguy]=0;
					targetrotation[whichguy]=90;
			...
			if(b!=-1&&rightshootpressed[whichguy]==0&&itemnum[whichguy]==5){
				...
				guyvely[b]=0; fallin[b]=0;
				activity[whichguy]=breakneckright; anim[whichguy]=16; targetanim[whichguy]=16;
				activity[b]=brokeneckright; anim[b]=17; targetanim[b]=17;
				 usingjetpack[b]=0;
				targetframe[whichguy]=1; targetframe[b]=1;
				frame[whichguy]=0; frame[b]=0;
				time[whichguy]=0; time[b]=0;
			...
			if(b!=-1&&rightshootpressed[whichguy]==0&&itemnum[whichguy]==2&&activity[whichguy]!=ducking){
				...
				guyvely[b]=0; fallin[b]=0;
				 usingjetpack[b]=0;
				activity[whichguy]=impaleright; anim[whichguy]=18; targetanim[whichguy]=18;
				activity[b]=impaledright; anim[b]=19; targetanim[b]=19;
				targetframe[whichguy]=1; targetframe[b]=1;
				frame[whichguy]=0; frame[b]=0;
				time[whichguy]=0; time[b]=0;
				}
			...
			if(b!=-1&&rightshootpressed[whichguy]==0&&itemnum[whichguy]==3&&activity[whichguy]!=ducking){
				...
				guyvely[b]=0; fallin[b]=0;
				 usingjetpack[b]=0;
				activity[whichguy]=swordthrowright; anim[whichguy]=20; targetanim[whichguy]=20;
				activity[b]=swordthrownright; anim[b]=21; targetanim[b]=21;
				targetframe[whichguy]=1; targetframe[b]=1;
				frame[whichguy]=0; frame[b]=0;
				time[whichguy]=0; time[b]=0;
			...
			if(b!=-1&&rightshootpressed[whichguy]==0&&itemnum[whichguy]<=3&&itemnum[whichguy]>=2&&activity[whichguy]==ducking){
				...
				guyvely[b]=0; fallin[b]=0;
				 usingjetpack[b]=0;
				activity[whichguy]=duckimpaleright; anim[whichguy]=38; targetanim[whichguy]=38;
				activity[b]=duckimpaledright; anim[b]=39; targetanim[b]=39;
				targetframe[whichguy]=1; targetframe[b]=1;
				frame[whichguy]=0; frame[b]=0;
				time[whichguy]=0; time[b]=0;
				c=(guyx[whichguy]+595)/10;
	...
	if ( CheckAIKey(whichguy, kLeftShootKey[whichguy] ) ){
		if(usingjetpack[whichguy]==0){facing[whichguy]=0;}
		if(activity[whichguy]==jetleft||activity[whichguy]==jetright||activity[whichguy]==jetup){if(itemnum[whichguy]>=1&&itemnum[whichguy]<=5){activity[whichguy]=jetidle;}}
		if(activity[whichguy]==idle1||activity[whichguy]==idle2||activity[whichguy]==swordidleright||activity[whichguy]==swordidleleft||activity[whichguy]==grenidleright||activity[whichguy]==grenidleleft||activity[whichguy]==falling||activity[whichguy]==jetidle||activity[whichguy]==ducking){
			...
			if(b==-1&&ammo[whichguy]&&itemnum[whichguy]==1&&Walls[h-2][c]==0){
				...
				if(Walls[a][c]==0&&activity[whichguy]==falling){
					activity[whichguy]=shootright; targetframe[whichguy]=0; targetanim[whichguy]=30; time[whichguy]=100; 
					targetrotation[whichguy]=-90;
				}
				if(Walls[a][c]==0&&Walls[f][c]==0&&activity[whichguy]==jetidle){
					activity[whichguy]=shootright; targetframe[whichguy]=0; targetanim[whichguy]=30; time[whichguy]=100; 
			...
			if(b==-1&&itemnum[whichguy]==5&&ammo[whichguy]>0&&Walls[h-2][c]==0){
				...
				c=guyy[whichguy]/-20-.5+40;
				if(Walls[a][c]==0&&activity[whichguy]!=falling&&activity[whichguy]!=jetidle&&activity[whichguy]!=ducking){
					activity[whichguy]=shootright; targetframe[whichguy]=0; targetanim[whichguy]=32; time[whichguy]=70; targetrotation[whichguy]=0;
				}
				if(Walls[a][c]==0&&activity[whichguy]==falling){
					activity[whichguy]=shootright; targetframe[whichguy]=0; targetanim[whichguy]=30; time[whichguy]=100; 
					targetrotation[whichguy]=-90;
			...
			if(b!=-1&&leftshootpressed[whichguy]==0&&itemnum[whichguy]==2&&activity[whichguy]!=ducking){
				...
				guyvelx[b]=0;
				guyvely[b]=0; fallin[b]=0;
				activity[whichguy]=impaleleft; anim[whichguy]=18; targetanim[whichguy]=18;
				activity[b]=impaledleft; anim[b]=19; targetanim[b]=19;
				targetframe[whichguy]=1; targetframe[b]=1;
				frame[whichguy]=0; frame[b]=0;
				time[whichguy]=0; time[b]=0;
				 usingjetpack[b]=0;
			...
			if(b!=-1&&leftshootpressed[whichguy]==0&&itemnum[whichguy]==3&&activity[whichguy]!=ducking){
				...
				guyvelx[b]=0;
				guyvely[b]=0; fallin[b]=0;
				activity[whichguy]=swordthrowleft; anim[whichguy]=20; targetanim[whichguy]=20;
				activity[b]=swordthrownleft; anim[b]=21; targetanim[b]=21;
				targetframe[whichguy]=1; targetframe[b]=1;
				frame[whichguy]=0; frame[b]=0;
				time[whichguy]=0; time[b]=0;
				c=(guyx[whichguy]+595)/10;
			...
			if(b!=-1&&leftshootpressed[whichguy]==0&&itemnum[whichguy]<=3&&itemnum[whichguy]>=2&&activity[whichguy]==ducking){
				...
				guyvely[b]=0; fallin[b]=0;
				 usingjetpack[b]=0;
				activity[whichguy]=duckimpaleleft; anim[whichguy]=38; targetanim[whichguy]=38;
				activity[b]=duckimpaledleft; anim[b]=39; targetanim[b]=39;
				targetframe[whichguy]=1; targetframe[b]=1;
				frame[whichguy]=0; frame[b]=0;
				time[whichguy]=0; time[b]=0;
				c=(guyx[whichguy]+595)/10;
	...
	if ( CheckAIKey(whichguy, kLeftKey[whichguy] ) ){
		facing[whichguy]=0;
		if(activity[whichguy]==idle1||activity[whichguy]==idle2||activity[whichguy]==swordidleright||activity[whichguy]==swordidleleft||activity[whichguy]==grenidleright||activity[whichguy]==grenidleleft){
			if(anim[whichguy]!=3){activity[whichguy]=runleft; targetframe[whichguy]=1; frame[whichguy]=0;  anim[whichguy]=2; targetanim[whichguy]=2; time[whichguy]=100;}
		}
		if(activity[whichguy]==onrope){
			activity[whichguy]=falling; targetframe[whichguy]=0; guyvelx[whichguy]=-.1; guyvely[whichguy]=-.1;
		}
		if(activity[whichguy]==ducking){
			activity[whichguy]=leftroll; targetframe[whichguy]=1; time[whichguy]=0;
		}
		if(activity[whichguy]==jetidle||activity[whichguy]==falling||activity[whichguy]==jetup){
			if(jetpack[whichguy]){activity[whichguy]=jetleft; targetframe[whichguy]=0; time[whichguy]=0; targetanim[whichguy]=34;}
		}
	}
	//**********RIGHT KEY************//
	if ( CheckAIKey(whichguy, kRightKey[whichguy] ) ){
		facing[whichguy]=1;
		if(activity[whichguy]==idle1||activity[whichguy]==idle2||activity[whichguy]==swordidleright||activity[whichguy]==swordidleleft||activity[whichguy]==grenidleright||activity[whichguy]==grenidleleft){
			if(anim[whichguy]!=3){activity[whichguy]=runright;  targetframe[whichguy]=1; frame[whichguy]=0; anim[whichguy]=2; targetanim[whichguy]=2; time[whichguy]=100;}
		}
		if(activity[whichguy]==onrope){
			activity[whichguy]=falling; targetframe[whichguy]=0; guyvelx[whichguy]=.1; guyvely[whichguy]=-.1;
		}
		if(activity[whichguy]==ducking){
			activity[whichguy]=rightroll; targetframe[whichguy]=1; time[whichguy]=0;
		}
		if(activity[whichguy]==jetidle||activity[whichguy]==falling||activity[whichguy]==jetup){
			if(jetpack[whichguy]){activity[whichguy]=jetright; targetframe[whichguy]=0; time[whichguy]=0; targetanim[whichguy]=34;}
		}
	}
	//**********DOWN KEY************//
	if ( CheckAIKey(whichguy, kDownKey[whichguy] ) ){
		if(activity[whichguy]==idle1||activity[whichguy]==idle2||activity[whichguy]==swordidleright||activity[whichguy]==swordidleleft||activity[whichguy]==grenidleright||activity[whichguy]==grenidleleft||activity[whichguy]==getupfromback||activity[whichguy]==getupfromfront){
			if(Map[guymapx[whichguy]][guymapy[whichguy]+1]!=2&&Map[guymapx[whichguy]][guymapy[whichguy]+1]!=3){activity[whichguy]=ducking; targetframe[whichguy]=1; frame[whichguy]=0; targetanim[whichguy]=3; anim[whichguy]=3; time[whichguy]=100;}
			if(Map[guymapx[whichguy]][guymapy[whichguy]+1]==2||Map[guymapx[whichguy]][guymapy[whichguy]+1]==3){
				frame[whichguy]=0; targetframe[whichguy]=2;
				guyy[whichguy]-=4;
		         activity[whichguy]=downrope; anim[whichguy]=5; targetanim[whichguy]=5; guyx[whichguy]=guymapx[whichguy]*10-590; time[whichguy]=80;
		    }
		}
		if(activity[whichguy]==ducking&&frame[whichguy]==2){targetframe[whichguy]=2;}
		if(activity[whichguy]==onrope){
			if(Map[guymapx[whichguy]][guymapy[whichguy]+1]==2||Map[guymapx[whichguy]][guymapy[whichguy]+1]==3||Map[guymapx[whichguy]][guymapy[whichguy]]==3||Map[guymapx[whichguy]][guymapy[whichguy]]==2){
				guyvelx[whichguy]=0; guyvely[whichguy]=0;
				frame[whichguy]=2; targetframe[whichguy]=2;
				activity[whichguy]=downrope; targetanim[whichguy]=5; anim[whichguy]=5; targetframe[whichguy]=2; guyx[whichguy]=guymapx[whichguy]*10-590; time[whichguy]=0;
			}
		}
		if(activity[whichguy]==falling||activity[whichguy]==leftflip||activity[whichguy]==rightflip){
			if(guyvelx[whichguy]!=0){
				if(Map[guymapx[whichguy]][guymapy[whichguy]]==3||Map[guymapx[whichguy]][guymapy[whichguy]]==2){
					if (activity[whichguy]!=onrope){frame[whichguy]=2; targetframe[whichguy]=2;}
					guyvelx[whichguy]=0; guyvely[whichguy]=0;
					activity[whichguy]=downrope; targetanim[whichguy]=5;anim[whichguy]=5;  targetframe[whichguy]=2; guyx[whichguy]=guymapx[whichguy]*10-590; time[whichguy]=0;
					rotation[whichguy]=0;
				}
		...
		if(activity[whichguy]==runleft){
			activity[whichguy]=leftroll; targetframe[whichguy]=0; time[whichguy]=0;
		}
		if(activity[whichguy]==runright){
			activity[whichguy]=rightroll; targetframe[whichguy]=0; time[whichguy]=0;
		}
	}
	//**********UP KEY************//
	if ( CheckAIKey(whichguy, kUpKey[whichguy] )){
		...
		if(activity[whichguy]==idle1||activity[whichguy]==idle2||activity[whichguy]==swordidleright||activity[whichguy]==swordidleleft||activity[whichguy]==grenidleright||activity[whichguy]==grenidleleft||activity[whichguy]==onrope||activity[whichguy]==falling||activity[whichguy]==leftflip||activity[whichguy]==rightflip){
			if(Map[guymapx[whichguy]][guymapy[whichguy]]==2||Map[guymapx[whichguy]][guymapy[whichguy]]==3||Map[guymapx[whichguy]][guywillbe]==2||Map[guymapx[whichguy]][guywillbe]==3){
				if (activity[whichguy]!=onrope){frame[whichguy]=0; targetframe[whichguy]=1;}
				guyvelx[whichguy]=0; guyvely[whichguy]=0;
				activity[whichguy]=climbrope; targetanim[whichguy]=5; time[whichguy]=100; guyx[whichguy]=guymapx[whichguy]*10-590;
			}
		}
		if(activity[whichguy]==idle1||activity[whichguy]==idle2||activity[whichguy]==swordidleright||activity[whichguy]==swordidleleft||activity[whichguy]==grenidleright||activity[whichguy]==grenidleleft){
			if(uppressed[whichguy]==0){
				activity[whichguy]=jumpstraight; frame[whichguy]=1; anim[whichguy]=6; targetanim[whichguy]=6; targetframe[whichguy]=2; time[whichguy]=0;
			}
		}
		if(activity[whichguy]==runleft&&uppressed[whichguy]==0){
			activity[whichguy]=leftflip; anim[whichguy]=7; targetanim[whichguy]=7; frame[whichguy]=0; targetframe[whichguy]=1; time[whichguy]=0;
		}
		if(activity[whichguy]==runright&&uppressed[whichguy]==0){
			activity[whichguy]=rightflip; anim[whichguy]=7; targetanim[whichguy]=7; frame[whichguy]=0; targetframe[whichguy]=1; time[whichguy]=0;
		}
		if(activity[whichguy]==falling&&jetpack[whichguy]==1){
			if(jetpack[whichguy]){activity[whichguy]=jetup; targetanim[whichguy]=33; targetframe[whichguy]=0; time[whichguy]=100;}
		}
		if(activity[whichguy]==jetidle&&jetpack[whichguy]==1){
			if(jetpack[whichguy]){activity[whichguy]=jetup; targetanim[whichguy]=33; targetframe[whichguy]=0; time[whichguy]=100;}
		}
		uppressed[whichguy]=1;
		if (activity[whichguy]==climbrope&&Map[guymapx[whichguy]][guymapy[whichguy]]==0){activity[whichguy]=onrope;}
		if (activity[whichguy]==climbrope&&Map[guymapx[whichguy]][guywillbe]==1&&Map[guymapx[whichguy]][guymapy[whichguy]]!=0&&Map[guymapx[whichguy]][guymapy[whichguy]+1]!=1){activity[whichguy]=falling; nosound=1; guyvely[whichguy]=-.1; frame[whichguy]=0;}
	}
...
void	EventLoop( void )
{
	...
	while ( gQuit == false )
	{
		...
		gamespeed=1;
		if (slowdown){gamespeed=.05;} //slowmotion toggle
		if (freezetime){gamespeed=0;} //freeze time toggle
		
		multiplier=oldmult*gamespeed;
		
		for(x=0;x<kMaxLightning;x++){
			Lightning[x].brightness-=multiplier/100;
			if(Lightning[x].brightness<=0){Lightning[x].brightness=0;}
		}
		...
		for(x=0;x<numplayers;x++){
			...
			multiplier/=10;
			for(a=0;a<10;a++){
				if(freezetime==0&&computercontrolled[x]==0){DoPlayerStuff(x);}
				if(freezetime==0&&computercontrolled[x]!=0){DoAIPlayerStuff(x);}
			}
			if(x!=1||mapeditor==0){if(dead[x]==0||activity[x]!=deadfacedown||activity[x]!=deadfaceup||health[x]>=0){ if(computercontrolled[x]==0){DoKeys(x);} if(computercontrolled[x]){DoAI(x); DoAIKeys(x);}}}
		}
		...
		for(x=0;x<numplayers;x++){
			if(activity[x]==jetimpaledright||activity[x]==jetimpaledleft&&attach[x]!=-1){guyx[x]=guyx[attach[x]]; guyy[x]=guyy[attach[x]];guyvelx[x]=guyvelx[attach[x]]; guyvely[x]=guyvely[attach[x]];}
		}
		...
		*/
		...
		if ( IsKeyDown( theKeyMap, 34 ) ){
			...
			if(freezetime==1){cameray -= multiplier/3;}
			}
		...
		if ( IsKeyDown( theKeyMap, 38 ) ){
			...
			if(freezetime==1){camerax += multiplier/3;}
			}
